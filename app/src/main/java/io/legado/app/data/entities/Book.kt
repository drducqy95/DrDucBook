...
data class Book(
    ...
    fun getQuickTranslationPronounModeOverride(): QuickTranslationPronounMode? =
        getVariable(QUICK_TRANSLATION_PRONOUN_MODE_KEY)
            .takeIf(String::isNotBlank)
            ?.let(QuickTranslationPronounMode::from)

    fun getQuickTranslationPronounModeOverrideValue(): String =
        getVariable(QUICK_TRANSLATION_PRONOUN_MODE_KEY).takeIf(String::isNotBlank).orEmpty()

    fun putQuickTranslationPronounModeOverride(value: String?) {
        val clean = value
            ?.takeIf(String::isNotBlank)
    ...
    fun setTranslationMode(enabled: Boolean) {
        ...
        if (!enabled) config.readerContentMode = ReaderContentMode.RAW.name
    }
    ...
    fun setReaderContentMode(mode: ReaderContentMode) {
        config.readerContentMode = mode.name
        config.translationMode = mode != ReaderContentMode.RAW
    }

    fun getReaderContentMode(fallback: ReaderContentMode = ReaderContentMode.RAW): ReaderContentMode {
        return config.readerContentMode
            ?.let { stored -> ReaderContentMode.entries.firstOrNull { it.name == stored } }
    ...
    companion object {
        const val QUICK_TRANSLATION_PRONOUN_MODE_KEY = "quickTranslationPronounMode"
        const val hTag = 2L
