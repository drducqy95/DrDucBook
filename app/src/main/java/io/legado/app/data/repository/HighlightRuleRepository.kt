...
class HighlightRuleRepository(
    ...
    fun sanitizeRule(rule: HighlightRule): HighlightRule {
        ...
        val sampleText = rule.sampleText
        val id = rule.id.ifBlank {
        ...
        return HighlightRule(
            ...
            underlineMode = rule.underlineMode.coerceIn(0, 5),
            underlineColor = rule.underlineColor,
            underlineWidth = rule.underlineWidth.coerceIn(0.1f, 10f),
            underlineOffset = rule.underlineOffset.coerceIn(0f, 20f),
            underlineSvgPath = rule.underlineSvgPath,
            bgImage = rule.bgImage?.takeIf { it.isNotBlank() },
            bgImageFit = rule.bgImageFit.coerceIn(0, 2),
            bgImageScale = rule.bgImageScale.coerceIn(0.1f, 5f),
            configName = rule.configName?.takeIf { it.isNotBlank() },
            fontPath = rule.fontPath?.takeIf { it.isNotBlank() },
        )
    }
