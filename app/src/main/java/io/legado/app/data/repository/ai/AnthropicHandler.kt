...
class AnthropicHandler : AiProtocolHandler {
    ...
    override suspend fun fetchModels(provider: AiProviderConfig): Result<List<AiAvailableModel>> =
        withContext(Dispatchers.IO) {
            runCatching {
                if (provider.protocol == AiProtocol.CLAUDE_SUBSCRIPTION) emptyList()
                else fetchModelsInternal(provider)
            }
        }
    ...
    private suspend fun generateInternal(request: AiGenerateRequest): AiGenerateResponse {
        ...
        return retryWithBackoff(maxAttempts = keyRotator.attemptsAtLeast(3), keyRotator = keyRotator) {
            val response = aiOkHttpClient.newCallStrResponse {
                url(anthropicMessagesUrl(provider))
                postJson(GSON.toJson(body))
                addHeaders(anthropicHeaders(provider, keyRotator.currentKey))
            }
...
private fun anthropicMessagesUrl(provider: AiProviderConfig): String =
    provider.baseUrl.trimEnd('/') + provider.messagesPath +
        if (provider.protocol == AiProtocol.CLAUDE_SUBSCRIPTION) "?beta=true" else ""
...
internal fun anthropicHeaders(
    ...
    provider.customHeaders.forEach { (name, value) ->
        put(name, value.replace("{apiKey}", credential).replace("${'$'}API_KEY", credential))
    }
    provider.runtimeMetadata["deviceId"]?.takeIf(String::isNotBlank)?.let { deviceId ->
        put("X-Msh-Platform", "9router")
        put("X-Msh-Version", AiProviderRegistry.UPSTREAM_VERSION)
        put("X-Msh-Device-Name", "Android")
        put("X-Msh-Device-Model", "Android")
        put("X-Msh-Device-Id", deviceId)
    }
    put("anthropic-version", "2023-06-01")
    put("Content-Type", "application/json")
    if (provider.protocol == AiProtocol.CLAUDE_SUBSCRIPTION) {
        put("Authorization", "Bearer $credential")
        put(
            "anthropic-beta",
        ...
        put("User-Agent", "claude-cli/2.1.92 (external, sdk-cli)")
        put("x-app", "cli")
        put("x-stainless-helper-method", "stream")
        put("x-stainless-retry-count", "0")
        put("x-stainless-runtime-version", "v24.14.0")
        put("x-stainless-package-version", "0.80.0")
        put("x-stainless-runtime", "node")
        put("x-stainless-lang", "js")
        put("x-stainless-arch", "arm64")
        put("x-stainless-os", "Android")
        put("x-stainless-timeout", "600")
    } else if (provider.authType == AiProviderAuthType.BEARER) {
        put("Authorization", "Bearer $credential")
    } else {
        put("x-api-key", credential)
    }
}
