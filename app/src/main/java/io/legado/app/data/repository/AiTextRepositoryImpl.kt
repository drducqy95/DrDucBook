package io.legado.app.data.repository

import io.legado.app.data.repository.ai.AiProviderRegistry
import io.legado.app.data.repository.ai.AnthropicHandler
import io.legado.app.data.repository.ai.AntigravityHandler
import io.legado.app.data.repository.ai.CommandCodeHandler
import io.legado.app.data.repository.ai.GeminiHandler
import io.legado.app.data.repository.ai.OpenAiChatHandler
import io.legado.app.data.repository.ai.OpenAiResponsesHandler
import io.legado.app.domain.gateway.AiStreamEvent
import io.legado.app.domain.gateway.AiTextGateway
import io.legado.app.domain.model.AiAvailableModel
import io.legado.app.domain.model.AiGenerateRequest
import io.legado.app.domain.model.AiGenerateResponse
import io.legado.app.domain.model.AiProviderFailureClassifier
import io.legado.app.domain.model.AiProviderConfig
import kotlinx.coroutines.CancellationException
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.flow.Flow
import kotlinx.coroutines.flow.flow
import kotlinx.coroutines.flow.flowOn
import kotlinx.coroutines.withContext

class AiTextRepositoryImpl(
    localAiEngineGateway: io.legado.app.domain.gateway.LocalAiEngineGateway,
) : AiTextGateway {

    private val registry = AiProviderRegistry(
        handlers = listOf(
            OpenAiChatHandler(),
            OpenAiResponsesHandler(),
            AnthropicHandler(),
            GeminiHandler(),
            AntigravityHandler(),
            CommandCodeHandler(),
            io.legado.app.data.repository.ai.LocalGgufHandler(localAiEngineGateway),
        )
    )

    override suspend fun generate(request: AiGenerateRequest): Result<AiGenerateResponse> =
        withContext(Dispatchers.IO) {
            try {
                registry.handlerFor(request.model.provider.protocol).generate(request).fold(
                    onSuccess = { Result.success(it) },
                    onFailure = { error ->
                        if (error is CancellationException) throw error
                        Result.failure(classify(error, request))
                    },
                )
            } catch (error: CancellationException) {
                throw error
            } catch (error: Throwable) {
                Result.failure(classify(error, request))
            }
        }

    override fun generateStream(request: AiGenerateRequest): Flow<AiStreamEvent> = flow {
        try {
            registry.handlerFor(request.model.provider.protocol).stream(request) { emit(it) }
        } catch (error: CancellationException) {
            throw error
        } catch (error: Throwable) {
            throw classify(error, request)
        }
    }.flowOn(Dispatchers.IO)

    override suspend fun fetchModels(provider: AiProviderConfig): Result<List<AiAvailableModel>> =
        withContext(Dispatchers.IO) {
            try {
                registry.handlerFor(provider.protocol).fetchModels(provider).fold(
                    onSuccess = { Result.success(it) },
                    onFailure = { error ->
                        if (error is CancellationException) throw error
                        Result.failure(
                            AiProviderFailureClassifier.classify(
                                error = error,
                                provider = provider.name,
                                model = "danh sách model",
                            )
                        )
                    },
                )
            } catch (error: CancellationException) {
                throw error
            } catch (error: Throwable) {
                Result.failure(
                    AiProviderFailureClassifier.classify(
                        error = error,
                        provider = provider.name,
                        model = "danh sách model",
                    )
                )
            }
        }

    private fun classify(error: Throwable, request: AiGenerateRequest) =
        AiProviderFailureClassifier.classify(
            error = error,
            provider = request.model.provider.name,
            model = request.model.modelId,
        )
}
