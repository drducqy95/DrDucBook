...
interface AiProfileDao {
    ...
    @Query("select * from ai_task_presets where taskType = :taskType and enabled = 1 order by sortNumber, createdAt limit 1")
    suspend fun getFirstEnabledPreset(taskType: String): AiTaskPreset?

    @Query(
        """
        select * from ai_task_presets
        where taskType = :taskType and enabled = 1 and id != :excludedPresetId
        order by sortNumber, createdAt
        limit 1
        """
    )
    suspend fun getFirstEnabledPresetExcluding(
        taskType: String,
    ...
    @Query("DELETE FROM ai_task_presets WHERE modelProfileId = :modelProfileId")
    suspend fun deletePresetsByModel(modelProfileId: String)

    @Query(
        """
        DELETE FROM ai_task_presets
        WHERE modelProfileId IN (
            SELECT id FROM ai_model_profiles WHERE providerId = :providerId
        )
        """
    )
    suspend fun deletePresetsByProvider(providerId: String)

    @Query("UPDATE ai_task_presets SET isDefault = 0 WHERE taskType = :taskType")
    suspend fun clearDefaultPresets(taskType: String)

    @Query(
        """
        UPDATE ai_task_presets
        SET isDefault = 1, enabled = 1, updatedAt = :updatedAt
        WHERE id = :presetId
        """
    )
    suspend fun markPresetDefault(presetId: String, updatedAt: Long)

    @Transaction
    suspend fun setDefaultPreset(preset: AiTaskPreset) {
        clearDefaultPresets(preset.taskType)
        markPresetDefault(preset.id, System.currentTimeMillis())
    }

    @Transaction
    suspend fun upsertTaskPreset(preset: AiTaskPreset) {
        if (preset.isDefault) {
            clearDefaultPresets(preset.taskType)
        }
        insertPreset(preset)
