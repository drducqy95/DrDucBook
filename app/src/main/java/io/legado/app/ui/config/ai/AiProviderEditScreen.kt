...
fun AiProviderEditScreen(
    ...
    val providerPresetValues = arrayOf("") +
            state.providerPresets
                ...
                .map { it.id }
                .toTypedArray()
    val translationPromptEntries = state.translationPrompts
        .map { prompt -> "${prompt.name} · ${prompt.modelLabel}" }
        .toTypedArray()
    val translationPromptValues = state.translationPrompts
        .map(AiProviderPromptUi::id)
        .toTypedArray()
    val providerModelOptions = remember(
        state.providerModels,
        state.fetchedModels,
        state.providerName,
    ) {
        val fetchedModelIds = state.fetchedModels.mapTo(hashSetOf(), AiFetchedModelUi::id)
        state.providerModels.map { model ->
            AiModelPickerOptionUi(
                ...
                isMissing = fetchedModelIds.isNotEmpty() && model.modelId !in fetchedModelIds,
            )
        }.toImmutableList()
    }
    val fallbackModelOptions = remember(
        state.fallbackModels,
        state.fetchedModels,
        state.providerName,
    ) {
        val fetchedModelIds = state.fetchedModels.mapTo(hashSetOf(), AiFetchedModelUi::id)
        state.fallbackModels.map { model ->
            AiModelPickerOptionUi(
                id = model.modelProfileId,
                providerName = state.providerName,
                modelName = model.modelName,
    ...
    var showFallbackComboSheet by remember { mutableStateOf(false) }
        ...
        LazyColumn(
            ...
            item {
                SplicedColumnGroup(title = stringResource(R.string.ai_provider_models)) {
                    ClickableSettingItem(
                        title = stringResource(R.string.ai_model_database),
                        description = modelSummary(state.providerModels.size, state.providerModels),
                        onClick = { showModelPickerSheet = true },
                    )
                    ClickableSettingItem(
                        title = stringResource(R.string.ai_translation_fallback_combo),
                        description = modelSummary(state.fallbackModels.size, state.fallbackModels),
                        onClick = { showFallbackComboSheet = true },
                    )
                    ClickableSettingItem(
            ...
            item {
                SplicedColumnGroup(title = stringResource(R.string.translation_prompt)) {
                    if (state.translationPrompts.isNotEmpty()) {
                        DropdownListSettingItem(
                            title = stringResource(R.string.translation_prompt),
                            selectedValue = state.selectedTranslationPromptId,
                            displayEntries = translationPromptEntries,
                            entryValues = translationPromptValues,
                            description = stringResource(R.string.ai_provider_translation_prompt_summary),
                            onValueChange = {
                                onIntent(AiProviderEditIntent.SelectTranslationPrompt(it))
                            },
                        )
                    }
                    ClickableSettingItem(
    ...
    AiModelPickerSheet(
        show = showModelPickerSheet,
        title = stringResource(R.string.ai_provider_models),
        selectedModelId = state.fallbackModels.firstOrNull()?.modelProfileId.orEmpty(),
        models = providerModelOptions,
        onDismissRequest = { showModelPickerSheet = false },
        onSelect = { model ->
            onIntent(AiProviderEditIntent.EditModel(model.id))
        },
        onManualEntry = { onIntent(AiProviderEditIntent.AddModel) },
    )

    AiModelPickerSheet(
        show = showFallbackComboSheet,
        title = stringResource(R.string.ai_translation_fallback_combo),
        selectedModelId = state.fallbackModels.firstOrNull()?.modelProfileId.orEmpty(),
        models = fallbackModelOptions,
        onDismissRequest = { showFallbackComboSheet = false },
        onSelect = { model ->
            onIntent(AiProviderEditIntent.EditModel(model.id))
        },
    )
...
@Composable
private fun modelSummary(size: Int, models: List<AiProviderModelUi>): String {
    return when {
        size <= 0 -> stringResource(R.string.ai_model_not_configured)
        size == 1 -> models.first().modelName
        else -> {
            val preview = models.take(3).joinToString(" · ") { it.modelName }
