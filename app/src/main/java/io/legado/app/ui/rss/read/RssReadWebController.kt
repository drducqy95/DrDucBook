    ...
    webView.webViewClient = object : WebViewClient() {
        ...
        private fun handleRedirect(view: WebView, fromUrl: String?, toUrl: String): Boolean {
            ...
            return when (redirectPolicyProvider()) {
                ...
                RedirectPolicy.BLOCK_ALL -> {
                    context.toastOnUi(R.string.redirect_blocked)
                    true
                }
                ...
                RedirectPolicy.BLOCK_CROSS_ORIGIN -> {
                    if (crossOrigin) {
                        context.toastOnUi(R.string.cross_origin_redirect_blocked)
                        true
                    } else false
                }
                ...
                RedirectPolicy.ASK_SAME_DOMAIN_BLOCK_CROSS -> {
                    if (crossOrigin) {
                        context.toastOnUi(R.string.external_redirect_blocked)
                        true
                    } else {
                        askUser(fromUrl, toUrl) { if (it) view.loadUrl(toUrl) }
                        true
        ...
        private fun askUser(fromUrl: String?, toUrl: String, onResult: (Boolean) -> Unit) {
            AlertDialog.Builder(context)
                ...
                .setMessage(
                    context.getString(
                        R.string.redirect_request_message,
                        fromUrl ?: context.getString(R.string.unknown_source_url),
                        toUrl
                    )
                )
                .setPositiveButton(android.R.string.ok) { _, _ -> onResult(true) }
                .setNegativeButton(R.string.refuse) { _, _ -> onResult(false) }
                .setCancelable(true)
                .show()
        }
        ...
        override fun onPageFinished(view: WebView, url: String?) {
            ...
            CookieManager.syncFromWebView(url, viewModel.rssSource?.sourceUrl)
