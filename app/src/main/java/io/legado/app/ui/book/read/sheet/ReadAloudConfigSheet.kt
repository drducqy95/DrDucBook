...
import io.legado.app.domain.model.ExternalTtsVoiceAsset
...
fun SpeakEngineConfigSheet(
    ...
    var showLocalTtsCatalog by remember { mutableStateOf(false) }
    ...
    LocalTtsVoiceCatalogDialog(
        show = showLocalTtsCatalog,
        onDismissRequest = { showLocalTtsCatalog = false },
        onOpenUrl = { url ->
            showLocalTtsCatalog = false
            onIntent(ReadBookIntent.OpenLocalTtsModelCatalogItem(url))
        },
    )
    ...
    AppModalBottomSheet(
        ...
        endAction = {
            ...
            Box {
                ...
                RoundDropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }) {
                    ...
                    RoundDropdownMenuItem(
                        text = stringResource(R.string.import_local_tts_model),
                        onClick = {
                            expanded = false
                            if (!state.isImportingLocalTtsModel) {
                                onIntent(ReadBookIntent.ImportLocalTtsModel)
                            }
                        },
                    )
                    RoundDropdownMenuItem(
                        text = stringResource(R.string.download_local_tts_model_catalog),
                        onClick = {
                            expanded = false
                            showLocalTtsCatalog = true
                        },
                    )
                    RoundDropdownMenuItem(
        ...
        LazyColumn(
            ...
            if (state.isImportingLocalTtsModel) {
                item {
                    TinyClickableSettingItem(
                        title = stringResource(R.string.importing_local_tts_model),
                        description = stringResource(R.string.importing_local_tts_model_summary),
                        onClick = {},
                    )
                }
            }
            items(items) { item ->
                ...
                TinyClickableSettingItem(
                    ...
                    } else if (item.localModelId != null) {
                        stringResource(R.string.local_onnx_tts_model)
                    } else {
                        null
                    },
                    ...
                    trailingContent = if (httpTtsId != null || item.localModelId != null) {
                        {
                            Row(horizontalArrangement = Arrangement.spacedBy(2.dp)) {
                                ...
                                SmallTonalButton(
                                    onClick = {
                                        if (httpTtsId != null) {
                                            onIntent(ReadBookIntent.DeleteHttpTts(httpTtsId))
                                        } else {
                                            item.localModelId?.let {
                                                onIntent(ReadBookIntent.DeleteLocalTtsModel(it))
                                            }
                                        }
                                    },
...
@Composable
private fun LocalTtsVoiceCatalogDialog(
    show: Boolean,
    onDismissRequest: () -> Unit,
    ...
    AppAlertDialog(
        show = show,
        onDismissRequest = onDismissRequest,
        title = stringResource(R.string.local_tts_voice_catalog),
        text = stringResource(R.string.local_tts_voice_catalog_summary),
        ...
        content = {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                TinyClickableSettingItem(
                    ...
                    onClick = { onOpenUrl(ExternalAssetCatalog.ttsPiperVoiceFolderUrl) },
                )
                ExternalAssetCatalog.releaseEligibleTtsVoiceCatalog.forEach { asset ->
                    LocalTtsVoiceCatalogItem(asset = asset, onOpenUrl = onOpenUrl)
                }
            }
        },
    )
...
@Composable
private fun LocalTtsVoiceCatalogItem(
    asset: ExternalTtsVoiceAsset,
    onOpenUrl: (String) -> Unit,
) {
    val supportLabel = if (asset.importSupported) {
    ...
    TinyClickableSettingItem(
        ...
        description = "${asset.engine} - $supportLabel - ${formatAssetSize(asset.sizeBytes)}",
        onClick = { onOpenUrl(asset.downloadUrl) },
    )
}

private fun formatAssetSize(sizeBytes: Long): String =
    String.format(Locale.US, "%.1f MiB", sizeBytes / 1024f / 1024f)
