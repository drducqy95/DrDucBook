...
import io.legado.app.domain.usecase.AiChatAgentRun
import io.legado.app.domain.usecase.PendingToolRun
import io.legado.app.domain.usecase.ToolTraceBuilder
import io.legado.app.ui.ai.bubble.ChatBubbleSessionStore
import io.legado.app.ui.ai.bubble.ChatBubbleMessagePreview
...
class AiChatViewModel(
    ...
    private var activeToolLoopGuard = AgentToolLoopGuard()
    private var thinkingStartTime: Long = 0L
    private val bubbleControllerOwnerId = UUID.randomUUID().toString()
    ...
    init {
        ...
        observeBubbleSession()
        observeBubbleCancelRequests()
        observeBubbleTranscript()
        ChatBubbleSessionStore.registerController(bubbleControllerOwnerId)
    }

    private fun observeBubbleCancelRequests() {
        viewModelScope.launch {
            ChatBubbleSessionStore.cancelRequests.collect {
                stopGenerating()
            }
        }
        viewModelScope.launch {
            ChatBubbleSessionStore.sendRequests.collect { content ->
                sendMessage(content)
            }
        }
    }

    private fun observeBubbleTranscript() {
        viewModelScope.launch {
            _uiState
                .map { state ->
                    val messages = buildList {
                        state.messages.takeLast(5).forEach { message ->
                            add(
                                ChatBubbleMessagePreview(
                                    role = message.role,
                                    content = message.content.take(BUBBLE_MESSAGE_PREVIEW_CHARS),
                                )
                            )
                        }
                            ...
                            add(
                                ChatBubbleMessagePreview(
                                    ...
                                    content = message.content
                                        ...
                                        .take(BUBBLE_MESSAGE_PREVIEW_CHARS),
                                )
                            )
                        }
                    }
                    BubbleTranscriptSignal(state.currentConversationId, messages)
                }
    ...
    private fun observeBubbleSession() {
        viewModelScope.launch {
            _uiState
                .map { state ->
                    BubbleSessionSignal(
                        ...
                        waitingApproval = state.pendingToolConfirmation != null,
                    )
                }
                .distinctUntilChanged()
                .collect { signal ->
                    when {
                        signal.waitingApproval -> ChatBubbleSessionStore.markWaitingApproval(signal.conversationId)
                        signal.isSending -> ChatBubbleSessionStore.markRunning(signal.conversationId)
                        else -> ChatBubbleSessionStore.markFinished(signal.conversationId)
                    }
                }
        }
    }
    ...
    fun onIntent(intent: AiChatIntent) {
        when (intent) {
            ...
            is AiChatIntent.SelectToolApprovalScope -> selectToolApprovalScope(intent.scope)
    ...
    private fun selectConversation(id: String) {
        streamingJob?.cancel()
        pendingToolRun = null
        activeToolLoopGuard = AgentToolLoopGuard()
        ...
        viewModelScope.launch {
            ...
            val selectedRoute = preset?.runtimeOptions?.routeProfileId
                ?.takeIf(String::isNotBlank)
                ?.let { routeId ->
                    aiRouterGateway.observeSnapshot().first().routes
                        .firstOrNull { it.id == routeId && it.enabled }
                }
            val providerName = if (selectedRoute != null) {
                ""
            } else {
                preset?.model?.provider?.name.orEmpty()
            }
            val modelName = selectedRoute?.name ?: preset?.model?.displayName.orEmpty()
    ...
    private fun confirmPendingTool() {
        ...
        val approvalScope = _uiState.value.pendingToolConfirmation?.approvalScope
            ?: AgentApprovalScope.ONE_TIME
        pendingToolRun = null
        streamingJob = viewModelScope.launch {
            _uiState.update {
                it.copy(isSending = true, pendingToolConfirmation = null)
            }
            continueAfterToolApproval(pending, approvalScope)
        }
    }

    private fun selectToolApprovalScope(scope: AgentApprovalScope) {
        _uiState.update { state ->
            state.copy(
                pendingToolConfirmation = state.pendingToolConfirmation?.copy(
                    approvalScope = scope,
                )
            )
        }
    ...
    private fun rejectPendingTool() {
        val pending = pendingToolRun ?: return
        pendingToolRun = null
        val rejectionText = appCtx.getString(R.string.ai_tool_rejected_reply)
        ...
        streamingJob = viewModelScope.launch {
            pending.proposal?.let { generationUseCase.rejectToolProposal(it) }
            _uiState.update {
                it.copy(
                    isSending = false,
                    pendingToolConfirmation = null,
            ...
            persistAgentRun(
                run = pending.agentRun,
                status = AgentRunStatus.FINAL,
                finalText = assistantText,
                reasoning = pending.fullReasoning,
    ...
    private fun regenerateMessage(messageId: String) {
        ...
        streamingJob = viewModelScope.launch {
            ...
            var agentRun: AiChatAgentRun? = null
            try {
                val state = _uiState.value
                ...
                val activeRun = generationUseCase.startAgentRun(request, conversationId)
                agentRun = activeRun
                ...
                if (!waitingForToolConfirmation) {
                    agentRun?.let { run ->
                        persistAgentRun(
                            run = run,
                            status = when {
                                wasCancelled -> AgentRunStatus.CANCELLED
                                failureMessage != null -> AgentRunStatus.ERROR
                                else -> AgentRunStatus.FINAL
                            },
                            finalText = assistantContent.orEmpty(),
                            reasoning = fullReasoning.toString(),
                            toolTrace = toolTrace,
    ...
    private fun sendMessage(rawContent: String) {
        ...
        streamingJob = viewModelScope.launch {
            ...
            try {
                ...
                val activeRun = generationUseCase.startAgentRun(request, conversationIdForMsg)
                agentRun = activeRun
                setStreamingPlaceholder()
                try {
                    collectStream(request, fullText, fullReasoning, toolTrace)
    ...
    private suspend fun continueAfterToolApproval(
        ...
        try {
            ...
            val proposal = checkNotNull(pending.proposal) {
                "Approved tool run is missing its permission proposal"
            }
            val followUpRequest = generationUseCase.executeApprovedToolCalls(
            ...
            failureMessage = e.message ?: "AI tool failed"
            ChatBubbleSessionStore.markError(currentConversationId.value, failureMessage.orEmpty())
            _effects.tryEmit(AiChatEffect.ShowMessage(failureMessage))
        } finally {
            val assistantContent = if (waitingForToolConfirmation) {
            ...
            if (!waitingForToolConfirmation && pending.conversationId != null && assistantContent != null) {
                val duration = _uiState.value.streamingMessage?.thinkingDuration ?: 0
                runCatching {
                    aiChatGateway.saveMessage(
                        conversationId = pending.conversationId,
    ...
    private suspend fun continueToolRounds(
        ...
        while (true) {
            ...
            check(currentRound < AiChatGenerationUseCase.MAX_TOOL_ROUNDS) {
                "Chatbot exceeded the safe tool-call round limit"
            }
            if (toolCalls.any { generationUseCase.requiresConfirmation(it.name, conversationId) }) {
                val proposal = generationUseCase.createToolProposal(
                    conversationId = conversationId,
                    toolCalls = toolCalls,
                )
                ...
                _uiState.update {
                    it.copy(
                        isSending = false,
                        pendingToolConfirmation = buildAiToolConfirmation(
                            toolCalls = toolCalls,
    ...
    private suspend fun persistAgentRun(
        run: AiChatAgentRun,
        status: AgentRunStatus,
        finalText: String,
        reasoning: String,
        toolTrace: ToolTraceBuilder,
        pendingProposal: io.legado.app.domain.agent.AgentActionProposal? = null,
        errorMessage: String? = null,
    ) {
        withContext(NonCancellable) {
            runCatching {
                generationUseCase.saveAgentRun(
                    run = run,
                    status = status,
                    finalText = finalText,
                    reasoning = reasoning,
    ...
    private fun PendingToolRun.deniedToolParts(): List<AiMessagePart> {
        ...
        return toolTrace.toParts().map { part ->
            if (part is AiMessagePart.Tool &&
                ...
                part.copy(
                    output = appCtx.getString(R.string.ai_tool_rejected_output),
    ...
    private data class BubbleSessionSignal(
        val conversationId: String?,
        val isSending: Boolean,
        val waitingApproval: Boolean,
    )

    private data class BubbleTranscriptSignal(
        val conversationId: String?,
        val messages: List<ChatBubbleMessagePreview>,
    )

    override fun onCleared() {
        ChatBubbleSessionStore.unregisterController(bubbleControllerOwnerId)
    }

    private companion object {
        const val BUBBLE_MESSAGE_PREVIEW_CHARS = 320
    }
...
internal fun combineAiAssistantFailure(
    text: String,
    failureText: String?,
): String? {
    val content = text.trimEnd()
    val failure = failureText?.trim().orEmpty()
    return when {
        content.isNotBlank() && failure.isNotBlank() -> "$content\n\n$failure"
