...
class FilePickerViewModel(application: Application) : BaseViewModel(application) {
    ...
    fun createFolder(name: String) {
        execute {
            val dir = lastDir ?: throw NoStackTraceException(
                context.getString(R.string.parent_folder_not_found)
            )
            val folder = File(dir, name)
            if (!folder.canonicalPath.contains(dir.canonicalPath)) {
                throw NoStackTraceException(context.getString(R.string.invalid_file_name))
            }
            folder.mkdir()
        }.onSuccess {
            upFiles(lastDir)
