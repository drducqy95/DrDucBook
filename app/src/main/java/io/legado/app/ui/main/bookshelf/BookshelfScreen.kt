...
fun BookshelfScreen(
    ...
    onBookVeryLongClick: (BookUiItem) -> Unit,
    ...
    DisposableEffect(lifecycleOwner, viewModel) {
        val observer = LifecycleEventObserver { _, event ->
            if (event == Lifecycle.Event.ON_RESUME) {
                viewModel.refreshDynamicUiTranslations()
            }
        }
    ...
    AppScaffold(
        appearanceTarget = AppearanceTarget.BOOKSHELF,
                ...
                actions = {
                    ...
                    AnimatedVisibility(visible = isEditMode) {
                        TopBarActionButton(
                            onClick = {
                                if (selectedBookUrls.isNotEmpty()) {
                                    viewModel.showOverlay(BookshelfOverlay.BatchDeleteConfirmDialog)
                                }
                            },
                ...
                if (uiState.groups.isEmpty() && !uiState.isSearch) {
                    ...
                    if (isUsingStandaloneSearchGroup) {
                        BookshelfPage(
                            ...
                            onBookVeryLongClick = onBookVeryLongClick,
                            isCurrentPage = true,
...
private fun BookshelfOverlays(
    ...
    AppAlertDialog(
        show = activeOverlay == BookshelfOverlay.BatchDeleteConfirmDialog,
        onDismissRequest = { viewModel.dismissOverlay() },
        title = stringResource(R.string.delete_selected_books),
        text = stringResource(R.string.delete_selected_books_confirm, selectedBookUrls.size),
        confirmText = stringResource(R.string.delete),
        onConfirm = {
            viewModel.dismissOverlay()
            viewModel.deleteSelectedBooks(selectedBookUrls)
        },
        dismissText = stringResource(android.R.string.cancel),
        onDismiss = { viewModel.dismissOverlay() },
    )
            ...
            itemsIndexed(displayBooks, key = { _, item -> item.book.bookUrl }) { index, bookUi ->
                ...
                ReorderableItem(
                    ...
                    BookItem(
                        ...
                        onVeryLongClick = if (canReorderBooks || uiState.isEditMode) {
                            null
                        } else {
