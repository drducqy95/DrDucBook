...
fun ShadowSetSheet(
    ...
    var shadowColor by remember { mutableIntStateOf(0) }
    var shadowRadius by remember { mutableFloatStateOf(0f) }
    var shadowDx by remember { mutableFloatStateOf(0f) }
    var shadowDy by remember { mutableFloatStateOf(0f) }
    var showColorPicker by remember { mutableStateOf(false) }

    LaunchedEffect(show) {
        if (show) {
            textShadow = ReadBookConfig.textShadow
            shadowColor = ReadBookConfig.durConfig.curTextShadowColor()
