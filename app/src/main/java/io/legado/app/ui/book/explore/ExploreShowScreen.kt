...
fun ExploreShowScreen(
    ...
    title: String = "",
    onBack: () -> Unit,
    onOpenReader: (SearchBook) -> Unit,
    onBookInfo: (SearchBook, String?) -> Unit,
    onQuickDictionary: (projectKey: String, initialText: String) -> Unit,
    ...
    LaunchedEffect(viewModel) {
        viewModel.effects.collect { effect ->
            when (effect) {
                is ExploreShowEffect.OpenReader -> onOpenReader(effect.book)
                is ExploreShowEffect.OpenBookInfo -> onBookInfo(
                    SearchBook(
                        name = effect.name,
                ...
                is ExploreShowEffect.OpenQuickDictionary -> onQuickDictionary(
                    effect.projectKey,
                    effect.initialText,
                )
    ...
    var previewOriginalBook by remember { mutableStateOf<SearchBook?>(null) }
    ...
    val canLoadMore = state.books.isNotEmpty() &&
        ...
        !state.isRefreshing &&
        state.errorMsg == null
    val endPullNestedScrollConnection = rememberExploreEndPullConnection(
        listState = listState,
        gridState = gridState,
        ...
        hasError = state.errorMsg != null,
        isEnd = state.isEnd,
        onForceLoadNext = { viewModel.onIntent(ExploreShowIntent.ForceLoadNext) },
    )
    ExploreShowLoadMoreDetector(
        listState = listState,
        gridState = gridState,
        isGridMode = isGridMode,
        enabled = canLoadMore,
        gridPreloadDistance = state.gridCount,
        onLoadMore = { viewModel.onIntent(ExploreShowIntent.LoadMore) },
    )
    ...
    AppModalBottomSheet(
        ...
        Row(
            ...
            AppText(
                text = stringResource(R.string.explore_grid_columns),
                style = LegadoTheme.typography.titleMedium.copy(fontWeight = FontWeight.Bold)
            )
            Spacer(modifier = Modifier.width(12.dp))
            TextCard(
                text = stringResource(R.string.explore_column_count, state.gridCount),
    ...
    ExploreKindSelectSheet(
        ...
        displayNameOverrides = state.kindDisplayNames,
    ...
    AppScaffold(
        ...
        topBar = {
            GlassMediumFlexibleTopAppBar(
                ...
                title = state.selectedKindTitle ?: state.title ?: title,
                navigationIcon = {
            ...
            Crossfade(
                ...
                if (isGrid) {
                    LazyVerticalGrid(
                        ...
                        modifier = Modifier
                            .fillMaxSize()
                            .nestedScroll(endPullNestedScrollConnection)
                        ...
                        itemsIndexed(
                            ...
                            ExploreBookGridItem(
                                ...
                                onLongClick = { book, coverKey ->
                                    viewModel.onIntent(
                                        ExploreShowIntent.OpenBookInfo(item.book, coverKey)
                                    )
                                },
                                onVeryLongClick = { _, _ ->
                                    viewModel.onIntent(
                                        ExploreShowIntent.SelectBookText(item.book, item.displayBook)
                                    )
                                },
    ...
    SearchBookPreviewSheet(
        ...
        onDismissRequest = {
            previewBook = null
            previewOriginalBook = null
        },
        onOpenDetail = { _, sharedCoverKey ->
            val sourceBook = previewOriginalBook
            previewBook = null
            previewOriginalBook = null
            sourceBook?.let { onBookInfo(it, sharedCoverKey) }
        },
        onAddToShelf = {
            previewOriginalBook?.let { sourceBook ->
                viewModel.onIntent(ExploreShowIntent.AddToShelf(sourceBook))
            }
        },
    )
}
...
@Composable
private fun rememberExploreEndPullConnection(
    ...
    val latestIsGridMode by rememberUpdatedState(isGridMode)
    val latestHasBooks by rememberUpdatedState(hasBooks)
    val latestIsLoading by rememberUpdatedState(isLoading)
    val latestIsRefreshing by rememberUpdatedState(isRefreshing)
    val latestHasError by rememberUpdatedState(hasError)
    val latestIsEnd by rememberUpdatedState(isEnd)
    val latestOnForceLoadNext by rememberUpdatedState(onForceLoadNext)
    return remember(listState, gridState) {
        object : NestedScrollConnection {
            ...
            override fun onPostScroll(
                ...
                if (available.y > 0f) continuationRequested = false
                val canScrollForward = if (latestIsGridMode) {
                    gridState.canScrollForward
                } else {
                    listState.canScrollForward
                }
                val shouldContinue = shouldRequestExploreContinuationOnEndPull(
                    hasUnconsumedUpwardDrag = available.y < 0f && !continuationRequested,
                    canScrollForward = canScrollForward,
                    hasBooks = latestHasBooks,
                    isLoading = latestIsLoading,
                    isRefreshing = latestIsRefreshing,
                    hasError = latestHasError,
                    isEnd = latestIsEnd,
                )
                if (shouldContinue) {
                    continuationRequested = true
                    latestOnForceLoadNext()
                }
                return Offset.Zero
            }
...
@Composable
private fun ExploreShowLoadMoreDetector(
    listState: LazyListState,
    gridState: LazyGridState,
    isGridMode: Boolean,
    enabled: Boolean,
    gridPreloadDistance: Int,
    onLoadMore: () -> Unit,
) {
    val latestOnLoadMore by rememberUpdatedState(onLoadMore)
    LaunchedEffect(listState, gridState, isGridMode, enabled, gridPreloadDistance) {
        ...
        snapshotFlow {
            if (isGridMode) {
                ...
                shouldLoadNextExplorePage(
                    ...
                    lastVisibleIndex = layoutInfo.visibleItemsInfo.maxOfOrNull { it.index } ?: -1,
                    preloadDistance = gridPreloadDistance,
                )
            } else {
...
internal fun shouldLoadNextExplorePage(
    ...
    lastVisibleIndex >= totalItemsCount - preloadDistance.coerceAtLeast(1)

internal fun shouldRequestExploreContinuationOnEndPull(
    hasUnconsumedUpwardDrag: Boolean,
    ...
    hasError: Boolean,
    isEnd: Boolean,
): Boolean = hasUnconsumedUpwardDrag &&
    !canScrollForward &&
    hasBooks &&
    !isLoading &&
    !isRefreshing &&
...
fun ExploreBookItem(
    ...
    onVeryLongClick: ((SearchBook, String?) -> Unit)? = null,
    ...
    SearchBookListItem(
        ...
        onVeryLongClick = onVeryLongClick,
        modifier = modifier,
        sharedTransitionScope = sharedTransitionScope,
        animatedVisibilityScope = animatedVisibilityScope,
        sharedCoverKey = sharedCoverKey
