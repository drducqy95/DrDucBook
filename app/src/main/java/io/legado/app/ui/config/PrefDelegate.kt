...
fun <T> prefDelegate(
    ...
    return object : PrefDelegate<T>, DefaultLifecycleObserver {
        ...
        init {
            // SharedPreferences is the synchronous compatibility store. DataStore catches up off-main.
            val initialValue = readFromSp() ?: defaultValue
            _value = mutableStateOf(initialValue)
            currentValue = initialValue

            scope.launch {
                readFromDs()?.takeIf { it != currentValue }?.let { dsValue ->
                    updateValue(dsValue)
        ...
        override fun setValue(thisRef: Any?, property: KProperty<*>, value: T) {
            if (currentValue != value) {
                ...
                // Persist to DataStore without blocking the caller or the Compose main thread.
                scope.launch {
                    runCatching {
                        when (value) {
