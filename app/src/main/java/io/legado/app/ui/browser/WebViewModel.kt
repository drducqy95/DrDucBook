...
class WebViewModel(application: Application) : BaseViewModel(application) {
    ...
    fun initData(
        ...
        execute {
            ...
            val url = intent.getStringExtra("url")
                ?: throw NoStackTraceException(context.getString(R.string.url_required))
    ...
    fun saveImage(webPic: String?) {
        ...
        execute {
            ...
            if (!success) throw Throwable(context.getString(R.string.save_image_failed))
        }.onError {
            ACache.get().remove(imagePathKey)
            context.toastOnUi(
                context.getString(R.string.save_image_failed_with_error, it.localizedMessage)
            )
        }.onSuccess {
