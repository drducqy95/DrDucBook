...
import com.drducbook.app.databinding.DialogMangaFooterSettingBinding
...
class MangaFooterSettingDialog :
    ...
    override fun onFragmentCreated(view: View, savedInstanceState: Bundle?) {
        ...
        MangaScrollMode.ALL.forEach { mode ->
            binding.chipGroupScrollMode.addView(Chip(requireContext()).apply {
                text = MangaScrollMode.labelOf(requireContext(), mode)
                isCheckable = true
                ...
                setOnLongClickListener {
                    ...
                    toastOnUi(R.string.set_global_default_mode)
                    true
                }
            })
        }
    ...
    private fun updateChapterText() {
        ...
        val label = if (!config.hideChapterLabel) getString(R.string.chapter_label) else ""
        val chapter = if (!config.hideChapter) "1/45 " else ""
        val name = if (!config.hideChapterName) "第三话 " else ""

        val progressLabel = if (!config.hideProgressRatioLabel) {
            getString(R.string.total_progress_label)
        } else ""
        val progress = if (!config.hideProgressRatio) "2.1%" else ""

        val pageLabel = if (!config.hidePageNumberLabel) {
            getString(R.string.page_count_label)
        } else ""
