...
class ReadMangaViewModel(
    ...
    private fun autoChangeSource(name: String, author: String) {
        ...
        execute {
            ...
            flow {
                ...
                context.toastOnUi(
                    context.getString(R.string.auto_change_source_failed_with_error, it.localizedMessage)
                )
            }.collect()
        }
    }
    ...
    fun removeCurrentNotShelfBook(
        ...
        Coroutine.async {
            ...
            context.toastOnUi(R.string.remove_temporary_manga_failed)
            onFailure()
        }
    }
    ...
    fun saveImageToGallery(url: String, folderName: String = "Legado") {
        viewModelScope.launch {
            try {
                ...
                if (success) {
                    context.toastOnUi(R.string.image_saved_to_gallery)
                } else {
                    context.toastOnUi(R.string.save_failed)
                }
            } catch (e: Exception) {
                context.toastOnUi(
                    context.getString(R.string.save_image_failed_with_error, e.localizedMessage)
                )
            }
        }
    }
