    ...
    private fun save(navigateBack: Boolean) {
        viewModelScope.launch {
            ...
            runCatching {
                ...
                _effects.tryEmit(
                    AiModelEditEffect.ShowMessage(appCtx.getString(R.string.ai_default_model_saved))
                )
                if (navigateBack) {
    ...
    private fun testConnection() {
        viewModelScope.launch {
            ...
            runCatching {
                ...
                _effects.tryEmit(
                    AiModelEditEffect.ShowMessage(appCtx.getString(R.string.ai_test_succeeded))
                )
            }.onFailure { error ->
                _effects.tryEmit(
                    AiModelEditEffect.ShowMessage(
                        error.message ?: appCtx.getString(R.string.ai_test_failed)
                    )
                )
            }
