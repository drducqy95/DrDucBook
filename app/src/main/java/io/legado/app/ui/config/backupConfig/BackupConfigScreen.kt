...
fun BackupConfigScreen(
    ...
    var pendingBackupMode by remember { mutableStateOf<String?>(null) }
    ...
    val selectBackupPathLauncher = rememberLauncherForActivityResult(
        ...
        val requestedMode = pendingBackupMode
        ...
        uri?.let {
            ...
            requestedMode?.let { mode ->
                val path = BackupConfig.backupPath.orEmpty()
                if (path.isNotEmpty()) {
                    showLoadingDialog = true
                    loadingText = context.getString(R.string.backup)
    ...
    BackupOptionSheet(
        ...
        onBackupToLocal = {
            ...
            executeBackup("local", context, viewModel, scope, snackbarHostState, {
                showLoadingDialog = false
            }, { showLoadingDialog = true; loadingText = it }, { mode ->
