...
object MainNavigator {
    ...
    fun initialBackStack(route: NavKey): List<NavKey> = when (route) {
        is MainRouteBrowser -> listOf(MainRouteHome, route)
        else -> listOf(route)
    }
    ...
    fun navigateToRoute(backStack: MutableList<NavKey>, route: NavKey) {
        ...
        when (route) {
            ...
            MainRouteSettingsAiRouter -> navigateToRoute(backStack, MainRouteAiRouter)

            MainRouteAiAgentDashboard,
            MainRouteAiCustomTools,
            MainRouteAiRouter -> {
                if (
                    currentRoute == MainRouteHome ||
                    currentRoute == MainRouteSettingsAi ||
                    currentRoute == MainRouteAiAgentDashboard
                ) {
                    backStack.add(route)
                } else {
                    backStack.clear()
            ...
            MainRouteMediaDownloads,
            MainRouteAudiobookImport -> {
                if (
                    currentRoute == MainRouteHome ||
            ...
            is MainRouteMediaPlayer -> {
                backStack.removeAll { it is MainRouteBrowser }
                val mediaCurrentRoute = backStack.lastOrNull()
                if (
                    mediaCurrentRoute == MainRouteHome ||
                    mediaCurrentRoute is MainRouteBookInfo ||
                    mediaCurrentRoute is MainRouteExploreShow
                ) {
                    backStack.add(route)
                } else {
                    backStack.clear()
            ...
            is MainRouteQuickDictionaryManager -> {
                if (
                    currentRoute == MainRouteHome ||
                    currentRoute is MainRouteBookInfo ||
                    currentRoute is MainRouteExploreShow ||
                    currentRoute is MainRouteQuickDictionaryManager
                ) {
                    backStack.add(route)
                } else {
                    backStack.clear()
    ...
    private fun resolveStartRoute(route: String?, intent: Intent?): MainRoute {
        return when (route) {
            ...
            MainRouteConst.ROUTE_SETTINGS_TTS_MODELS -> MainRouteSettingsTtsModels
            ...
            MainRouteConst.ROUTE_SETTINGS_ACCOUNT -> MainRouteSettingsAccount
            MainRouteConst.ROUTE_ASSET_DELIVERY -> intent?.getStringExtra(MainIntent.EXTRA_ASSET_URI)
                ?.takeIf { it.isNotBlank() }
                ?.let(::MainRouteAssetDelivery)
                ?: MainRouteHome
            MainRouteConst.ROUTE_SETTINGS_AI -> MainRouteSettingsAi
            MainRouteConst.ROUTE_SETTINGS_AI_ROUTER,
            MainRouteConst.ROUTE_AI_ROUTER -> MainRouteAiRouter
            MainRouteConst.ROUTE_AI_CHAT -> MainRouteAiChat
            MainRouteConst.ROUTE_AI_AGENT_DASHBOARD -> MainRouteAiAgentDashboard
            MainRouteConst.ROUTE_AI_CUSTOM_TOOLS -> MainRouteAiCustomTools
            ...
            MainRouteConst.ROUTE_SETTINGS_MLKIT_MODELS -> MainRouteSettingsMlKitModels
            MainRouteConst.ROUTE_VBOOK_IMPORT -> MainRouteVbookImport
            MainRouteConst.ROUTE_SOURCE_HEALTH -> MainRouteSourceHealth(
                sourceUrl = intent?.getStringExtra(MainIntent.EXTRA_SOURCE_URL)
            )
            MainRouteConst.ROUTE_BROWSER -> MainRouteBrowser(
                url = intent?.getStringExtra(MainIntent.EXTRA_BROWSER_URL)
            )
            ...
            MainRouteConst.ROUTE_MEDIA_PLAYER -> intent
                ...
                ?.let { bookUrl ->
                    MainRouteMediaPlayer(
                        ...
                        chapterIndex = intent.getIntExtra(
                            MainIntent.EXTRA_CHAPTER_INDEX,
                            -1,
                        ).takeIf { it >= 0 },
                    )
                } ?: MainRouteHome
            MainRouteConst.ROUTE_MEDIA_DOWNLOADS -> MainRouteMediaDownloads
            MainRouteConst.ROUTE_AUDIOBOOK_IMPORT -> MainRouteAudiobookImport
