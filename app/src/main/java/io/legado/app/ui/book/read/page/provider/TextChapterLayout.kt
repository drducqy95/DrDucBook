...
import io.legado.app.ui.book.read.page.entities.column.DropCapColumn
...
class TextChapterLayout(
    ...
    private val centerChapterTitle = isMiddleTitle || book.isEpub
    ...
    private suspend fun setTypeHtml(
        ...
        val spanned = DropCapHtmlSupport.parse(
            html = htmlContent,
            basePaint = textPaint,
            targetHeight = contentPaintTextHeight * DropCapHtmlSupport.visualLineCount,
            gapPx = contentPaint.textSize * 0.28f,
        )
        val dialogueMask = if (ReadConfig.italicizeDialogue) {
            DialogueStyleAnalyzer.italicMask(spanned.toString())
        } else {
            null
        }
        ...
        fun createLayout(indents: IntArray?): StaticLayout {
            val builder = StaticLayout.Builder.obtain(spanned, 0, spanned.length, textPaint, width)
        ...
        var staticLayout = createLayout(null)
        repeat(3) {
            val indents = DropCapHtmlSupport.lineIndents(spanned, staticLayout)
                ?: return@repeat
            staticLayout = createLayout(indents)
        }
        val tempPaint = TextPaint(textPaint)
        val dropCapSpans = spanned.getSpans(
            0,
            spanned.length,
        ...
        for (lineIndex in 0 until staticLayout.lineCount) {
            ...
            val dropCap = dropCapSpans.firstOrNull { span ->
                staticLayout.getLineForOffset(spanned.getSpanStart(span)) == lineIndex
            }
            if (dropCap != null) {
                val lastDropCapLine = (lineIndex + DropCapHtmlSupport.lineCount - 1)
                    .coerceAtMost(staticLayout.lineCount - 1)
                val requiredHeight = staticLayout.getLineBottom(lastDropCapLine) -
                    staticLayout.getLineTop(lineIndex)
                prepareNextPageIfNeed(durY + requiredHeight)
            }
            prepareNextPageIfNeed(durY + lineHeight)
            textLine.lineTop = paddingTop + durY
            textLine.lineBottom = textLine.lineTop + lineHeight
            textLine.lineBase = textLine.lineTop + staticLayout.getLineBaseline(lineIndex) - mLineTop

            val columns = mutableListOf<BaseColumn>()
            dropCap?.let { span ->
                val firstGlyphIndex = (lineStart until lineEnd).firstOrNull { index ->
                    !spanned[index].isWhitespace()
                }
                val topOffset = firstGlyphIndex?.let { index ->
                    val glyphEnd = index + Character.charCount(Character.codePointAt(spanned, index))
                    val bounds = Rect()
                    textPaint.getTextBounds(
                        spanned.toString(),
                        index,
                        glyphEnd.coerceAtMost(spanned.length),
                        bounds,
                    )
                    (
                        staticLayout.getLineBaseline(lineIndex) -
                            mLineTop +
                            bounds.top
                        ).coerceAtLeast(0f)
                } ?: 0f
                val column = DropCapColumn(
                    start = absStartX.toFloat(),
                    end = absStartX + span.indent.toFloat(),
                    charData = span.initial,
                    textSize = span.textSize,
                    topOffset = topOffset,
                    typeface = span.typeface,
                )
                columns += column
                lineText.append(span.initial)
                textLine.visualOverflowBottom =
                    (column.visualBottom - lineHeight).coerceAtLeast(0f)
            }
                ...
                if (!addedImage) {
                    columns.add(
                        TextHtmlColumn(
                            ...
                            textColor,
                            linkUrl,
                            italic = dialogueMask?.getOrNull(charIndex) == true,
                        )
                    )
                }
                charIndex++
            ...
            val epubAlignment = spanned.getSpans(
                lineStart,
                lineEnd.coerceAtLeast(lineStart + 1).coerceAtMost(spanned.length),
                EpubTextAlignmentSpan::class.java,
            ).lastOrNull()
            val allowsJustification = epubAlignment?.alignment != Layout.Alignment.ALIGN_CENTER &&
                epubAlignment?.alignment != Layout.Alignment.ALIGN_OPPOSITE
            if ((textFullJustify || epubAlignment?.justify == true) &&
                allowsJustification &&
                !textLine.isParagraphEnd
            ) {
                val availableWidth = calculateHtmlLineAvailableWidth(
                    visibleWidth = visibleWidth,
                    absoluteStartX = absStartX,
                    firstColumnStart = columns.firstOrNull()?.start,
                    fallbackLineLeft = lineLeft,
                )
                justifyHtmlLine(columns, textLine, availableWidth)
            } else {
    ...
    private fun extractTextSize(spanned: Spanned, index: Int, defaultSize: Float): Float {
        ...
        if (relativeSpans.isNotEmpty()) {
            val scale = relativeSpans.fold(1f) { value, span -> value * span.sizeChange }
                .coerceIn(0.5f, 4f)
            return defaultSize * scale
        }
        spanned.getSpans(index, index + 1, AbsoluteSizeSpan::class.java)
            .lastOrNull()
            ?.let { span ->
                return span.size.toFloat().coerceIn(defaultSize * 0.5f, defaultSize * 4f)
            }
        return defaultSize
    }
    ...
    private suspend fun setTypeText(
        ...
        var charStyles = applyHighlightRules(text, isTitle, offset)
        if (ReadConfig.italicizeDialogue && !isTitle) {
            charStyles = applyDialogueStyles(text, charStyles)
        }
        ...
        for (lineIndex in 0 until layout.lineCount) {
            ...
            when (lineIndex) {
                ...
                layout.lineCount - 1 -> {
                    val startX = if (
                        isTitle &&
                        (centerChapterTitle || emptyContent || isVolumeTitle
    ...
    private fun applyDialogueStyles(
        text: String,
        existing: Array<CharStyle?>?,
    ): Array<CharStyle?>? {
        val mask = DialogueStyleAnalyzer.italicMask(text)
        if (mask.none { it }) return existing
        val styles = Array<CharStyle?>(text.length) { index -> existing?.getOrNull(index) }
        mask.forEachIndexed { index, italic ->
            if (italic) {
                styles[index] = (styles[index] ?: CharStyle()).copy(italic = true)
            }
        }
        return styles
    }
...
internal fun calculateHtmlLineAvailableWidth(
    visibleWidth: Int,
    absoluteStartX: Int,
    firstColumnStart: Float?,
    fallbackLineLeft: Float,
): Int {
    val leadingInset = firstColumnStart
        ?.let { (it - absoluteStartX).coerceAtLeast(0f) }
        ?: fallbackLineLeft.coerceAtLeast(0f)
    return (visibleWidth - leadingInset).roundToInt().coerceAtLeast(1)
}
