...
import com.drducbook.app.databinding.DialogBookChangeSourceBinding
        ...
        if (it) {
            ...
            if (!searchScope.isAll()) {
                lifecycleScope.launch {
                    context?.alert(getString(R.string.source_search_empty_title)) {
                        setMessage(getString(R.string.source_search_empty, group))
                        cancelButton()
                        okButton {
...
//            val textColor = requireContext().getPrimaryTextColor(isLight)
    ...
    private fun performChangeSource(searchBook: SearchBook, isReplace: Boolean) {
        ...
        val coroutine = viewModel.getToc(book, { toc, source ->
            ...
            context?.toastOnUi(
                if (isReplace) R.string.change_source_failed else R.string.add_book_failed
            )
        })
