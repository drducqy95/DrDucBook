...
import com.drducbook.app.databinding.ActivityMangaBinding
import com.drducbook.app.databinding.ViewLoadMoreBinding
...
import io.legado.app.ui.ai.context.clearAiScreenContext
import io.legado.app.ui.ai.context.publishAiScreenContext
...
import io.legado.app.ui.book.read.manga.MangaTranslationOverlayController
import io.legado.app.ui.book.read.manga.MangaTranslationEditorEffect
import io.legado.app.ui.book.read.manga.MangaTranslationEditorIntent
import io.legado.app.ui.book.read.manga.MangaTranslationEditorSheet
...
class ReadMangaActivity : VMBaseActivity<ActivityMangaBinding, ReadMangaViewModel>(),
    ...
    private val translateMangaPageUseCase by inject<TranslateMangaPageUseCase>()
    private val quickTranslationGateway by inject<QuickTranslationGateway>()
    private val mangaTranslationCacheGateway by inject<MangaTranslationCacheGateway>()
    private val mangaTranslationExporter by inject<MangaTranslationExportRepository>()
    private val mangaTranslationEditorViewModel by viewModel<MangaTranslationEditorViewModel>()
    private var mangaEditorImageUrl: String? = null
    private var mangaEditorHost: ComposeView? = null
    private var pendingMangaExportPlan: MangaExportPlan? = null
    private val mangaTranslationController by lazy {
        MangaTranslationOverlayController(
            scope = lifecycleScope,
            translateMangaPage = translateMangaPageUseCase,
            quickTranslationGateway = quickTranslationGateway,
            cacheGateway = mangaTranslationCacheGateway,
            bookProvider = { ReadManga.book },
            onEditRequest = { imageUrl, result ->
                mangaEditorImageUrl = imageUrl
                mangaTranslationEditorViewModel.onIntent(
                    MangaTranslationEditorIntent.Load(result)
                )
            },
        )
    }
...
//            setLoadingTextColor(R.color.white)
    ...
    private val createMangaTranslationExport = registerForActivityResult(
        ActivityResultContracts.CreateDocument("application/vnd.comicbook+zip")
    ) { uri ->
        val plan = pendingMangaExportPlan
        pendingMangaExportPlan = null
        if (uri == null || plan == null) return@registerForActivityResult
        lifecycleScope.launch {
            runCatching {
                withContext(IO) {
                    val temporaryDirectory = File(cacheDir, "manga_translation_export").apply {
                        mkdirs()
                    }
                    val exported = mangaTranslationExporter.export(plan, temporaryDirectory).single()
                    try {
                ...
                toastOnUi(R.string.manga_translation_export_complete)
            }.onFailure { error ->
                toastOnUi(error.localizedMessage ?: getString(R.string.manga_translation_export_failed))
            }
        }
    }
    ...
    private fun initRecyclerView() {
        mAdapter.onImageReady = mangaTranslationController::onImageReady
        ...
        binding.webtoonFrame.longPressListener = {
            ...
            page?.let { item ->
                selector(
                    arrayListOf(
                        SelectItem(getString(R.string.save), "save"),
                        SelectItem(getString(R.string.share), "share")
                    )
                ) { _, option, _ ->
                    when (option.value) {
    ...
    override fun upContent() {
        lifecycleScope.launch {
            ...
            loadMoreView.noMore(getString(R.string.no_more_chapters))
                    } else if (nextFinish) {
    ...
    override fun loadFail(msg: String) {
        ...
        loadMoreView.error(null, getString(R.string.load_failed_click_retry))
            }
        }
    }

    override fun onDestroy() {
        mangaTranslationController.cancelAll()
        mangaEditorHost?.let { host -> (host.parent as? ViewGroup)?.removeView(host) }
        mangaEditorHost = null
        clearAiScreenContext()
        ReadManga.unregister(this)
        super.onDestroy()
    }

    private fun attachMangaTranslationEditor() {
        val parent = window.decorView as ViewGroup
        mangaEditorHost = ComposeView(this).apply {
            isVisible = false
            setViewCompositionStrategy(ViewCompositionStrategy.DisposeOnDetachedFromWindow)
            setContent {
                AppTheme {
                    val state by mangaTranslationEditorViewModel.uiState.collectAsStateWithLifecycle()
                    LaunchedEffect(state.visible) {
                        this@apply.isVisible = state.visible
                    }
                    LaunchedEffect(mangaTranslationEditorViewModel) {
                        mangaTranslationEditorViewModel.effects.collectLatest { effect ->
                            if (effect is MangaTranslationEditorEffect.Saved) {
                                mangaEditorImageUrl?.let { imageUrl ->
                                    mangaTranslationController.updateTranslation(imageUrl, effect.result)
                                }
                            }
                        }
                    }
                    MangaTranslationEditorSheet(
                        state = state,
                        onIntent = mangaTranslationEditorViewModel::onIntent,
                    )
                }
            }
        }.also { host ->
    ...
    private fun publishMangaContext() {
        val book = ReadManga.book
        publishAiScreenContext(
            screen = "ReadMangaActivity",
            attributes = mapOf(
                "bookUrl" to book?.bookUrl,
                "bookName" to book?.name,
                "author" to book?.author,
                "sourceOrigin" to book?.origin,
                "sourceName" to ReadManga.bookSource?.bookSourceName,
                "chapterIndex" to ReadManga.durChapterIndex.toString(),
                "chapterSize" to ReadManga.simulatedChapterSize.toString(),
                "chapterTitle" to ReadManga.curMangaChapter?.chapter?.title,
            )
        )
    }
    ...
    override fun changeTo(source: BookSource, book: Book, toc: List<BookChapter>) {
        if (book.isImage) {
            ...
            toastOnUi(R.string.selected_source_not_manga)
        }
    }
    ...
    override fun onCompatOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.menu_manga_translation -> {
                if (!FeatureFlags.mangaTranslation) return true
                ReadMangaConfig.mangaTranslationEnabled = !ReadMangaConfig.mangaTranslationEnabled
                item.isChecked = ReadMangaConfig.mangaTranslationEnabled
                if (!item.isChecked) mangaTranslationController.cancelAll()
                mAdapter.notifyItemRangeChanged(0, mAdapter.itemCount)
            }

            R.id.menu_manga_translation_script -> selectMangaTranslationScript()

            R.id.menu_manga_translation_vertical -> {
                ReadMangaConfig.mangaTranslationVerticalReading =
                    !ReadMangaConfig.mangaTranslationVerticalReading
                item.isChecked = ReadMangaConfig.mangaTranslationVerticalReading
                restartMangaTranslation()
            }

            R.id.menu_manga_translation_export -> prepareMangaTranslationExport()
    ...
    private fun selectMangaTranslationScript() {
        val scripts = MangaOcrScript.entries
        val labels = listOf(
            getString(R.string.manga_translation_script_latin),
            getString(R.string.manga_translation_script_chinese),
            getString(R.string.manga_translation_script_japanese),
            getString(R.string.manga_translation_script_korean),
        )
        selector(getString(R.string.manga_translation_script), labels) { _, index ->
            ReadMangaConfig.mangaTranslationScript = scripts[index].name
            restartMangaTranslation()
        }
    }

    private fun restartMangaTranslation() {
        mangaTranslationController.cancelAll()
        mAdapter.notifyItemRangeChanged(0, mAdapter.itemCount)
    }

    private fun prepareMangaTranslationExport() {
        lifecycleScope.launch {
            runCatching {
                mangaTranslationController.buildExportPlan(
                    ReadManga.book?.name ?: getString(R.string.manga_translation_mode)
                )
            }.onSuccess { plan ->
                pendingMangaExportPlan = plan
                createMangaTranslationExport.launch(plan.outputFileName())
            }.onFailure { error ->
                toastOnUi(error.localizedMessage ?: getString(R.string.manga_translation_export_failed))
            }
        }
    ...
    private fun upMenu(menu: Menu) {
        ...
        menu.findItem(R.id.menu_manga_translation).isChecked =
            ReadMangaConfig.mangaTranslationEnabled
        menu.findItem(R.id.menu_manga_translation).isVisible = FeatureFlags.mangaTranslation
        menu.findItem(R.id.menu_manga_translation_vertical).isVisible = FeatureFlags.mangaTranslation
        menu.findItem(R.id.menu_manga_translation_script).isVisible = FeatureFlags.mangaTranslation
        menu.findItem(R.id.menu_manga_translation_vertical).isChecked =
            ReadMangaConfig.mangaTranslationVerticalReading
        menu.findItem(R.id.menu_manga_translation_script).title = getString(
            R.string.manga_translation_script_current,
            ReadMangaConfig.mangaTranslationScript.lowercase().replaceFirstChar(Char::uppercase),
        )
    }
    ...
    override fun addToBookshelf(book: Book, toc: List<BookChapter>) {
        viewModel.addToBookshelf(book, toc) {
            toastOnUi(R.string.book_added_to_shelf)
        }
    }
