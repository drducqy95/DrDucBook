...
fun ExploreKindSelectSheet(
    ...
    displayNameOverrides: Map<String, String> = emptyMap(),
    ...
    val filteredKinds = remember(query, kinds, displayNameOverrides) {
        if (query.isBlank()) kinds
        else kinds.filter { kind ->
            kind.title.contains(query, ignoreCase = true) ||
                    displayNameOverrides[kind.title]?.contains(query, ignoreCase = true) == true ||
                    ...
                    Row(
                        ...
                        rowItems.forEach { (kind, span) ->
                            ...
                            ExploreKindMultiTypeItem(
                                ...
                                isMiuix = isMiuix,
                                displayNameOverride = displayNameOverrides[kind.title],
                                useCase = useCase
                            )
                        }
