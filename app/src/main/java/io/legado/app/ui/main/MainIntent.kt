...
object MainIntent {
    ...
    const val EXTRA_BROWSER_URL = "browserUrl"
    const val EXTRA_EXPLORE_URL = "exploreUrl"
    const val EXTRA_ASSET_URI = "assetUri"
    ...
    fun createAiChatIntent(context: Context): Intent {
        return createLauncherIntent(context).apply {
            putExtra(EXTRA_START_ROUTE, MainRouteConst.ROUTE_AI_CHAT)
        }
    }
    ...
    fun createMediaPlayerIntent(
        ...
        return createLauncherIntent(context).apply {
            putExtra(EXTRA_START_ROUTE, MainRouteConst.ROUTE_MEDIA_PLAYER)
    ...
    fun createBrowserIntent(context: Context, url: String? = null): Intent {
        return createLauncherIntent(context).apply {
            putExtra(EXTRA_START_ROUTE, MainRouteConst.ROUTE_BROWSER)
    ...
    fun createAssetDeliveryIntent(context: Context, rawUri: String): Intent {
        return createLauncherIntent(context).apply {
            putExtra(EXTRA_START_ROUTE, MainRouteConst.ROUTE_ASSET_DELIVERY)
            putExtra(EXTRA_ASSET_URI, rawUri)
        }
    }

    fun createSourceHealthIntent(context: Context, sourceUrl: String? = null): Intent {
        return createLauncherIntent(context).apply {
            putExtra(EXTRA_START_ROUTE, MainRouteConst.ROUTE_SOURCE_HEALTH)
            sourceUrl?.takeIf { it.isNotBlank() }?.let {
                putExtra(EXTRA_SOURCE_URL, it)
            }
        }
    }
    ...
    private fun routeForConfigTag(configTag: String?): String {
        return when (configTag) {
            ...
            ConfigTag.ACCOUNT_CONFIG -> MainRouteConst.ROUTE_SETTINGS_ACCOUNT
