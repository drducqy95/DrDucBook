...
data class BookshelfManageScreenExportConfig(
    ...
    val exportContentSource: String = EbookExportContentSource.BOTH.value,
    val exportCharset: String = "UTF-8",
    val bookExportFileName: String? = null,
    val episodeExportFileName: String = "",
    val vbookEbookUnlockCode: String? = null,
) {
...
sealed interface BookshelfManageScreenIntent {
    ...
    data class SetExportContentSource(val source: String) : BookshelfManageScreenIntent
    ...
    data class SetVbookEbookUnlockCode(val code: String?) : BookshelfManageScreenIntent
}
...
class BookshelfManageScreenViewModel(
        ...
        when (intent) {
            ...
            is BookshelfManageScreenIntent.SetExportUseReplace -> {
                ...
                val msg = context.getString(
                    if (intent.enabled) R.string.bookshelf_manage_replace_enabled
                    else R.string.bookshelf_manage_replace_disabled
                )
            ...
            is BookshelfManageScreenIntent.SetExportContentSource -> {
                bookshelfManageScreenConfig.exportContentSource = intent.source
                syncExportConfig()
            }
            ...
            is BookshelfManageScreenIntent.SetVbookEbookUnlockCode -> {
                bookshelfManageScreenConfig.vbookEbookUnlockCode = intent.code
                syncExportConfig()
            }
        }
    }
            ...
            CacheBook.downloadStateFlow.collect { downloadState ->
                ...
                val failureMsgs = downloadState.books.mapNotNull { (bookUrl, bookState) ->
                    val message = bookState.failureMessage ?: if (bookState.failedIndices.isNotEmpty()) {
                        context.getString(
                            R.string.bookshelf_manage_failed_chapters,
        ...
        _uiState.update {
            it.copy(
                exportConfig = BookshelfManageScreenExportConfig(
                    ...
                    exportContentSource = bookshelfManageScreenConfig.exportContentSource,
                    ...
                    vbookEbookUnlockCode = bookshelfManageScreenConfig.vbookEbookUnlockCode,
                )
            )
        }
    }
    ...
    private fun refreshGroupName(groupId: Long) {
        execute {
            ...
            title ?: context.getString(com.drducbook.app.R.string.no_group)
        }.onSuccess { groupName ->
    ...
    private fun toggleBookDownload(book: Book) {
        ...
        if (isBookDownloading(book.bookUrl) || pendingDownloadBookUrls.contains(book.bookUrl)) {
            ...
            execute {
                ...
                if (count <= 0) {
                    pendingDownloadBookUrls.remove(book.bookUrl)
                    downloadFailureMessages[book.bookUrl] = context.getString(
                        R.string.bookshelf_manage_no_cacheable_chapters
                    )
                    emitBookChanged(book.bookUrl)
                    _effects.tryEmit(
                        BookshelfManageScreenEffect.ShowMessage(
                            context.getString(R.string.bookshelf_manage_no_cacheable_chapters)
                        )
                    )
                }
            }.onError { error ->
                pendingDownloadBookUrls.remove(book.bookUrl)
                downloadFailureMessages[book.bookUrl] = error.localizedMessage
                    ?: context.getString(R.string.bookshelf_manage_unknown_error)
                emitBookChanged(book.bookUrl)
                _effects.tryEmit(
                    BookshelfManageScreenEffect.ShowMessage(
                        context.getString(
                            R.string.bookshelf_manage_cache_failed,
    ...
    private fun moveBooksToGroup(bookUrls: Set<String>, groupId: Long) {
        ...
        execute {
            ...
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(context.getString(
                R.string.bookshelf_manage_move_group_failed,
    ...
    private fun deleteBooks(bookUrls: Set<String>, deleteOriginal: Boolean) {
        ...
        execute {
            ...
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(
                context.getString(R.string.bookshelf_manage_delete_success)
            ))
        }.onError {
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(context.getString(
                R.string.bookshelf_manage_delete_failed,
                it.localizedMessage.orEmpty(),
            )))
        }
    }
    ...
    private fun clearCachesForBooks(bookUrls: Set<String>) {
        ...
        execute {
            ...
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(
                context.getString(R.string.bookshelf_manage_cache_cleared)
            ))
        }.onError {
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(context.getString(
                R.string.bookshelf_manage_clear_cache_failed,
                it.localizedMessage.orEmpty(),
            )))
        }
    }
    ...
    private fun downloadBooks(bookUrls: Set<String>, downloadAllChapters: Boolean) {
        ...
        execute {
            ...
            if (count > 0) {
                ...
                _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(
                    context.getString(R.string.bookshelf_manage_no_cacheable_books)
                ))
            }
            syncDownloadRunning()
        }.onError {
    ...
    private fun changeBookSource(
        ...
        execute {
            ...
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(
                context.getString(R.string.bookshelf_manage_change_source_complete)
            ))
        }.onError {
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(context.getString(
                R.string.bookshelf_manage_change_source_failed,
                it.localizedMessage.orEmpty(),
            )))
        }
    }
    ...
    private fun batchChangeBookSource(
        ...
        if (bookUrls.isEmpty()) {
            _uiState.update { it.copy(changeSourceError = context.getString(
                R.string.bookshelf_manage_no_books_selected
            )) }
            return
        }
        if (sources.isEmpty()) {
            _uiState.update { it.copy(changeSourceError = context.getString(
                R.string.bookshelf_manage_no_sources_selected
            )) }
            return
        }
        execute {
            ...
            _uiState.update {
                it.copy(
                    ...
                    changeSourceMessage = context.getString(
                        R.string.bookshelf_manage_find_start,
                        bookUrls.size,
                        sources.size,
                        concurrency,
                    ),
            ...
            _uiState.update {
                it.copy(
                    changeSourceMessage = context.getString(
                        R.string.bookshelf_manage_find_complete,
                        matchedCount,
                        notFoundCount,
                        skippedCount,
                    ),
            ...
            _uiState.update { state ->
                state.copy(
                    changeSourceError = context.getString(
                        R.string.bookshelf_manage_find_failed,
                        if (progress.isBlank()) "" else context.getString(
                            R.string.bookshelf_manage_progress_line,
                            progress,
                        ),
                        it.localizedMessage.orEmpty(),
                    )
    ...
    private fun migratePreviewItem(oldBookUrl: String) {
        ...
        execute {
            ...
            val chapters = changeBookSourceUseCase.loadCandidateChapters(
                candidate.source,
                candidate.book
            ) ?: error(context.getString(R.string.bookshelf_manage_toc_failed))
            ...
            emitBookChanged(result.book.bookUrl)
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(
                context.getString(R.string.bookshelf_manage_migration_complete)
            ))
        }.onError {
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(context.getString(
                R.string.bookshelf_manage_migration_failed,
                it.localizedMessage.orEmpty(),
            )))
        }
    }
    ...
    private fun addPreviewItemToShelf(oldBookUrl: String) {
        ...
        execute {
            ...
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(
                context.getString(R.string.bookshelf_manage_added_to_shelf)
            ))
        }.onError {
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(context.getString(
                R.string.bookshelf_manage_add_failed,
                it.localizedMessage.orEmpty(),
            )))
        }
    }
    ...
    private fun migrateAllPreviewItems() {
        ...
        execute {
            ...
            _uiState.update { it.copy(batchChangePreviewItems = emptyList()) }
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(
                context.getString(R.string.bookshelf_manage_batch_migration_complete)
            ))
        }.onError {
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(context.getString(
                R.string.bookshelf_manage_batch_migration_failed,
                it.localizedMessage.orEmpty(),
            )))
        }.onFinally {
            _uiState.update {
    ...
    private fun addAllPreviewItemsToShelf() {
        ...
        execute {
            ...
            _uiState.update { it.copy(batchChangePreviewItems = emptyList()) }
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(
                context.getString(R.string.bookshelf_manage_batch_add_complete)
            ))
        }.onError {
            _effects.tryEmit(BookshelfManageScreenEffect.ShowMessage(context.getString(
                R.string.bookshelf_manage_batch_add_failed,
                it.localizedMessage.orEmpty(),
            )))
        }.onFinally {
            _uiState.update {
