...
class BookInfoViewModel(
    ...
    private var currentBook: Book? = null
        set(value) {
            val previousFingerprint = field?.dynamicUiFingerprint()
            field = value
            observeReadRecordIfNeeded(value)
            val nextFingerprint = value?.dynamicUiFingerprint()
            if (previousFingerprint != nextFingerprint) {
                translatedBookUi = null
                translatedKindLabels = null
                translatedHighlightedTags = null
                translatedRelatedBooks = null
                dynamicUiTranslationJob?.cancel()
                value?.let(::scheduleDynamicUiTranslation)
            }
        }
    ...
    private var dynamicUiTranslationJob: Job? = null
    private var translatedBookUi: BookInfoBookUi? = null
    private var translatedKindLabels: List<String>? = null
    private var translatedHighlightedTags: List<HighlightedTag>? = null
    private var translatedRelatedBooks: List<RelatedBooksUi>? = null
        ...
        execute {
            ...
            if (dbBook != null) {
                ...
                if (searchBook != null) {
                    ...
                    currentBook ?: throw NoStackTraceException(
            ...
            context.toastOnUi(
                it.localizedMessage ?: context.getString(R.string.book_not_found)
            )
        ...
        when (intent) {
            ...
            is BookInfoIntent.AddSourceAsNewBook -> {
                addToBookshelf(intent.book, intent.toc) {
                    context.toastOnUi(R.string.book_added_to_shelf)
                }
            }
            ...
            is BookInfoIntent.StartEbookExport -> {
                val book = currentBook ?: return
                dismissSheet()
                emitEffect(
                    BookInfoEffect.OpenEbookExportDirectory(
                        bookUrl = book.bookUrl,
                        format = intent.format,
                        scope = intent.scope,
                        contentSource = intent.contentSource,
                        imageOptimization = intent.imageOptimization,
                        sendToKindle = intent.sendToKindle,
                    )
                )
            }
        }
    ...
    fun requestSourceVariableDialog() {
        execute {
            val source = bookSource ?: throw NoStackTraceException(
                context.getString(R.string.source_not_exist)
            )
            val comment = source.getDisplayVariableComment(
                context.getString(R.string.book_info_source_variable_default_comment)
            )
            ...
            context.toastOnUi(
                it.localizedMessage ?: context.getString(R.string.source_not_exist)
            )
        }
    }
    ...
    fun requestBookVariableDialog() {
        execute {
            ...
            val comment = source.getDisplayVariableComment(
                context.getString(R.string.book_info_book_variable_default_comment)
            )
            BookInfoEffect.ShowVariableDialog(
    ...
    fun syncFromRemote() {
        ...
        execute {
            ...
            syncUiState(isTocLoading = true)
            loadChapter(newBook)
            context.toastOnUi(R.string.sync_complete)
        }.onFinally {
            setBusy(false)
        }.onError {
    ...
    fun clearCache() {
        currentBook?.let { book ->
            execute {
                ...
                context.toastOnUi(
                    context.getString(
                        R.string.book_info_clear_cache_failed,
    ...
    private fun saveCoverToGallery(path: String) {
        ...
        execute {
            ...
            if (success) {
                ...
                context.toastOnUi(R.string.save_failed)
            }
        }.onFinally {
            setBusy(false)
        }.onError {
    ...
    fun addToBookshelf(book: Book, toc: List<BookChapter>, success: (() -> Unit)? = null) {
        execute {
            book.normalizeTypeFromSource(bookSource)
            book.removeType(BookType.notShelf)
            if (book.order == 0) {
                book.order = appDb.bookDao.minOrder - 1
            }
        ...
        if (book.isLocal) {
            ...
            WebBook.getBookInfo(scope, source, book, canReName = canReName)
                .onSuccess(IO) { loadedBook ->
                    loadedBook.normalizeTypeFromSource(source)
    ...
    private fun refreshMeta(book: Book) {
        execute {
            ...
            syncUiState()
            currentBook?.let(::scheduleDynamicUiTranslation)
        }
    }
    ...
    private fun loadWebFile(book: Book) {
        execute {
            val fileNameNoExtension = if (book.author.isBlank()) {
                ...
                context.getString(R.string.book_file_name_author, book.name, book.author)
            }
    ...
    private fun handleMenuAction(action: BookInfoMenuAction) {
        ...
        when (action) {
            ...
            BookInfoMenuAction.StoryMemory -> currentBook?.let { book ->
                emitEffect(BookInfoEffect.OpenStoryMemory(book.bookUrl))
            }
            ...
            BookInfoMenuAction.Upload -> uploadBook {
                context.toastOnUi(R.string.upload_success)
            }
            ...
            BookInfoMenuAction.ExportEbook -> setSheet(BookInfoSheet.EbookExport)
        }
    }
    ...
    private fun syncUiState(isTocLoading: Boolean = _uiState.value.isTocLoading) {
        _uiState.update {
            it.copy(
                book = translatedBookUi ?: currentBook?.toBookInfoBookUi(),
                hasChapters = currentChapterList.isNotEmpty(),
                tocLoadFailed = tocLoadFailed,
                webFiles = currentWebFiles,
                relatedBooks = (translatedRelatedBooks ?: currentRelatedBooks).toImmutableList(),
                highlightedTags = translatedHighlightedTags ?: currentHighlightedTags,
                kindLabels = translatedKindLabels ?: currentKindLabels,
    ...
    private fun scheduleRelatedBooksLoad(
        ...
        relatedBooksLoadJob = viewModelScope.launch {
            ...
            try {
                ...
                syncUiState()
                scheduleDynamicUiTranslation(book)
    ...
    private fun scheduleDynamicUiTranslation(book: Book) {
        ...
        val expectedFingerprint = book.dynamicUiFingerprint()
        val bookSnapshot = book.uiCopy()
        ...
        dynamicUiTranslationJob?.cancel()
        dynamicUiTranslationJob = viewModelScope.launch(IO) {
            ...
            translatedBookUi = translatedUi
            _uiState.update { it.copy(book = translatedUi) }

            val kindSnapshot = currentKindLabels
            val highlightedSnapshot = currentHighlightedTags
            val translatedKinds = kindSnapshot.map { translated(it).orEmpty() }
            val translatedHighlights = highlightedSnapshot.map { tag ->
                tag.copy(
                    matchedLabels = tag.matchedLabels.map { translated(it).orEmpty() },
                    title = translated(tag.title),
                )
            }
            if (currentBook?.dynamicUiFingerprint() != expectedFingerprint) return@launch
            translatedKindLabels = translatedKinds
            translatedHighlightedTags = translatedHighlights
            _uiState.update {
                it.copy(
                    kindLabels = translatedKinds,
                    highlightedTags = translatedHighlights,
                )
            }

            val relatedSnapshot = currentRelatedBooks
            val translatedModules = relatedSnapshot.map { module ->
                module.copy(
                    title = translated(module.title).orEmpty(),
                    books = module.books.map { sourceBook ->
                        translateDynamicBookUiUseCase.execute(sourceBook)
                    }.toImmutableList(),
                )
            }
            if (currentBook?.dynamicUiFingerprint() != expectedFingerprint) return@launch
            translatedRelatedBooks = translatedModules
            _uiState.update { it.copy(relatedBooks = translatedModules.toImmutableList()) }
        }
    }
