...
import com.drducbook.app.databinding.FragmentWebdavAuthBinding
...
class WebDavFragment : BaseFragment(R.layout.fragment_webdav_auth) {
        ...
        binding.btnRestore.setOnClickListener {
            ...
            lifecycleScope.launch {
                try {
                    ...
                    binding.btnRestore.setText(R.string.get_backup)
                }
            }
        }
    ...
    fun restore() {
        ...
        Coroutine.async {
            ...
            alert {
                ...
                setMessage(
                    getString(R.string.webdav_restore_fallback_message, it.localizedMessage)
                )
                okButton {
                    restoreFromLocal()
                }
    ...
    private suspend fun showRestoreDialog(context: Context) {
        ...
        if (webDavBackupUseCase.isJianGuoYun && names.size > 700) {
            context.toastOnUi(R.string.nutstore_backup_limit_warning)
        }
        if (names.isNotEmpty()) {
    ...
    private fun restoreWebDav(name: String) {
        ...
        val task = Coroutine.async {
            ...
            appCtx.toastOnUi(
                appCtx.getString(R.string.webdav_restore_failed, it.localizedMessage)
            )
        }.onFinally {
            waitDialog.dismiss()
        }
