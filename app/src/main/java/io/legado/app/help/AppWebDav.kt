    ...
    suspend fun testWebDav(): Boolean {
        return kotlin.runCatching {
            ...
            if (account.isNullOrEmpty() || password.isNullOrEmpty()) {
                appCtx.toastOnUi(R.string.webdav_credentials_required)
                return false
            }
            ...
            appCtx.toastOnUi(R.string.webdav_available)
            true
        }.getOrElse {
            it.printStackTrace()
            if (it !is WebDavException) {
                appCtx.toastOnUi(it.message ?: appCtx.getString(R.string.unknown_error))
            }
            false
        }
    }
