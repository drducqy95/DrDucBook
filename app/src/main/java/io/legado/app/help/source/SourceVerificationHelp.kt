...
object SourceVerificationHelp {
    ...
    /** Prevent background health checks from stealing the foreground with CAPTCHA dialogs. */
    private object NonInteractiveVerification : CoroutineContext.Element {
        override val key: CoroutineContext.Key<*> = Key

        object Key : CoroutineContext.Key<NonInteractiveVerification>
    }

    class VerificationCancelledException : CancellationException("Source verification cancelled")

    suspend fun <T> withoutInteractivePrompt(block: suspend () -> T): T =
        withContext(NonInteractiveVerification) { block() }

    private fun interactiveVerificationAllowed(): Boolean =
        rhinoContextOrNull?.coroutineContext?.get(NonInteractiveVerification.Key) == null
    ...
    fun getVerificationResult(
        ...
        if (!interactiveVerificationAllowed()) {
            throw NoStackTraceException(
                "CAPTCHA_REQUIRED: interactive verification is disabled during background source checks"
            )
        }
        ...
        clearResult(source.getKey())
        if (result.second.isEmpty()) throw VerificationCancelledException()
        return result
    }
    ...
    fun startBrowser(
        ...
        if (!interactiveVerificationAllowed()) {
            throw NoStackTraceException(
                "CAPTCHA_REQUIRED: interactive browser verification is disabled during background source checks"
            )
        }
