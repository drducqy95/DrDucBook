...
fun Book.getLocalUri(): Uri {
    ...
    if (!defaultBookDir.isNullOrBlank()) {
        ...
        if (!treeFileDoc.exists()) {
            appCtx.toastOnUi(R.string.book_storage_location_invalid)
        } else {
...
fun Book.normalizeTypeFromSource(source: BookSource?): Book {
    if (source == null || isLocal) return this
    val sourceType = source.getBookType() and BookType.allBookType
    if (sourceType == 0) return this
    val preservedFlags = type and BookType.allBookType.inv()
    type = preservedFlags or sourceType
    return this
}
