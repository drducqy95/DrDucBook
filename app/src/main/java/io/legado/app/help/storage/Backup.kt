...
object Backup : KoinComponent {
    ...
    private val metadataBackupFileNames by lazy {
        arrayOf(
            "bookshelf.json",
            "bookmark.json",
            "browserBookmarks.json",
            "sourceBookmarkPreferences.json",
            "bookGroup.json",
            "bookSource.json",
            "rssSources.json",
            "rssStar.json",
    ...
    private fun backupFileNames(profile: BackupContentProfile): List<String> {
        val names = metadataBackupFileNames.toMutableList()
        names += "backup-manifest.json"
        if (BackupArchivePolicy.includesDownloadedContent(profile)) {
            names += listOf(
                "appearance",
                "authoring",
                "bookChapters.json",
                "mediaDownloadTasks.json",
                "mediaDownloadItems.json",
            )
            names += BackupArchivePolicy.fullDirectories
        }
        return names
    }
    ...
    suspend fun backupLocked(
        ...
        path: String?,
        mode: String = "both",
        profile: BackupContentProfile = BackupContentProfile.FULL,
    ) {
        BackupRestoreLock.withLock {
            withContext(IO) {
                backup(context, path, mode, profile = profile)
            }
        }
    }

    suspend fun createArchiveLocked(
        context: Context,
        destination: File,
        profile: BackupContentProfile = BackupContentProfile.FULL,
    ) {
    ...
    private suspend fun backup(
        ...
        if (BackupArchivePolicy.includesDownloadedContent(profile)) {
            appearanceGateway.exportSnapshot(File(backupPath, "appearance"))
            AuthoringBackupFiles.copyValidatedSnapshot(
                source = File(appCtx.filesDir, "authoring"),
                destination = File(backupPath, "authoring"),
            )
            BackupArchivePolicy.fullDirectories.forEach { directoryName ->
                val source = File(appCtx.externalFiles, directoryName)
                if (source.exists()) {
                    source.copyRecursively(File(backupPath, directoryName), overwrite = true)
                }
            }
        }
        writeListToJson(appDb.bookDao.all, "bookshelf.json", backupPath)
        writeListToJson(appDb.bookmarkDao.all, "bookmark.json", backupPath)
        writeListToJson(appDb.browserBookmarkDao.allBookmarks, "browserBookmarks.json", backupPath)
        writeListToJson(
            appDb.browserBookmarkDao.allSourcePreferences,
            "sourceBookmarkPreferences.json",
            backupPath,
        )
        ...
        FileUtils.createFileIfNotExist(backupPath + File.separator + "backup-manifest.json")
            .writeText(GSON.toJson(BackupManifest(profile = profile)))
        if (BackupArchivePolicy.includesDownloadedContent(profile)) {
            writeListToJson(appDb.bookChapterDao.getAll(), "bookChapters.json", backupPath)
            writeListToJson(appDb.mediaDownloadDao.getAllTasks(), "mediaDownloadTasks.json", backupPath)
            writeListToJson(appDb.mediaDownloadDao.getAllItems(), "mediaDownloadItems.json", backupPath)
        }

        currentCoroutineContext().ensureActive()
        val zipFileName = getNowZipFileName()
        val paths = backupFileNames(profile)
            .map { backupPath + File.separator + it }
        ...
        if (ZipUtils.zipFiles(paths, zipFilePath)) {
            archiveDestination?.let { destination ->
                File(zipFilePath).copyTo(destination, overwrite = true)
            }
        ...
        FileUtils.delete(zipFilePath)
        currentCoroutineContext().ensureActive()
        if (profile == BackupContentProfile.FULL && (mode == "both" || mode == "webdav")) {
