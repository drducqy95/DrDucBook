...
object ReadStyleResolver {
    ...
    fun currentBackground(config: ReadBookConfig.Config): ReadBackground {
        appearanceReaderBackground()?.let { return it }
    ...
    private fun appearanceReaderBackground(): ReadBackground? {
        val mode = currentMode()
        if (mode == ReadStyleMode.EInk) return null
        val gateway = runCatching {
            GlobalContext.get().get<AppearanceGateway>()
        }.getOrNull() ?: return null
        val wallpaper = gateway.state.value.activeProfile.wallpaperFor(
            target = AppearanceTarget.READER,
            dark = mode == ReadStyleMode.Night,
        ) ?: return null
        val path = gateway.resolveAsset(wallpaper.assetId, wallpaper.legacyLocation)
            ?: return null
        return ReadBackground(type = 2, value = path)
    }
