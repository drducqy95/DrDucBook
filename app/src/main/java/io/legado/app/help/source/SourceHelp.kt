...
object SourceHelp {
    ...
    fun insertRssSource(vararg rssSources: RssSource) {
        ...
        rssSourcesGroup[true]?.forEach {
            appCtx.toastOnUi(
                appCtx.getString(R.string.adult_source_blocked, it.sourceName)
            )
        }
    ...
    fun insertBookSource(vararg bookSources: BookSource) {
        ...
        bookSourcesGroup[true]?.forEach {
            appCtx.toastOnUi(
                appCtx.getString(R.string.adult_source_blocked, it.bookSourceName)
            )
        }
    ...
    private fun deleteSourceHealthState(key: String) {
        appDb.bookSourceHealthDao.deleteBySourceUrlBlocking(key)
        appDb.sourceCheckDao.deleteRunsBySourceUrlBlocking(key)
    }
