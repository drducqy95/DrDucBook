...
class CrashHandler(val context: Context) : Thread.UncaughtExceptionHandler {
    ...
    companion object {
        ...
        fun saveCrashInfo2File(ex: Throwable): String {
            ...
            printWriter.close()
            val fullResult = writer.toString()
            val result = if (fullResult.length > MAX_CRASH_STACK_CHARS) {
                fullResult.take(MAX_CRASH_STACK_CHARS) + CRASH_LOG_TRUNCATED_SUFFIX
            } else {
                fullResult
            }
            sb.append(result)
        ...
        fun readCrashLog(fileName: String?): String? {
            return runCatching {
                ...
                file?.bufferedReader()?.use { reader ->
                    val result = StringBuilder(MAX_CRASH_REPORT_CHARS)
                    val buffer = CharArray(8_192)
                    while (result.length < MAX_CRASH_REPORT_CHARS) {
                        val remaining = MAX_CRASH_REPORT_CHARS - result.length
                        val count = reader.read(buffer, 0, minOf(buffer.size, remaining))
                        if (count < 0) break
                        result.append(buffer, 0, count)
                    }
                    if (result.length == MAX_CRASH_REPORT_CHARS && reader.read() >= 0) {
                        result.append(CRASH_LOG_TRUNCATED_SUFFIX)
                    }
                    result.toString()
                }
            }.getOrNull()
        ...
        private const val MAX_CRASH_STACK_CHARS = 200_000
        private const val MAX_CRASH_REPORT_CHARS = 220_000
        private const val CRASH_LOG_TRUNCATED_SUFFIX =
            "\n\n[Crash report truncated to keep the app responsive]"
