...
class ExploreBooksUseCase(
    ...
    private suspend fun resolveRequest(
        ...
        val source = args
            ...
            ?: base
        val url = moduleUrl?.takeIf(String::isNotBlank)
            ?: (if (key != null) source.searchUrl?.takeIf(String::isNotBlank) else null)
            ?: resolveDefaultExploreUrl(source)
            ?: throw NoExploreUrl(sourceUrl)
        return ExploreRequest(source, url)
    }

    private suspend fun resolveDefaultExploreUrl(source: BookSource): String? {
        gateway.getExploreKinds(source)
            .firstOrNull { it.type == ExploreKind.Type.url && !it.url.isNullOrBlank() }
            ?.url
            ?.let { return it }

        return source.exploreUrl
            ?.takeIf(String::isNotBlank)
            ?.takeUnless(::looksLikeExploreKindCollection)
    }

    private fun looksLikeExploreKindCollection(value: String): Boolean {
        val text = value.trim()
