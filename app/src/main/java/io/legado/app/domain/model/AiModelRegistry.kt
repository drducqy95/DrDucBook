...
object AiModelRegistry {
    ...
    private val patterns = listOf(
        ...
        // DeepSeek V4 variants expose their chain of thought separately and can spend the
        // complete output budget before producing final content.
        ModelPattern(
            tokens = listOf("deepseek", "v4"),
            capabilities = setOf(AiCapability.TOOLS, AiCapability.REASONING, AiCapability.STREAMING)
        ),
        // OpenCode's Big Pickle currently exposes reasoning separately from final content. Without
        // the reasoning capability, short transformation requests can exhaust max_tokens before
        // the provider emits visible output.
        ModelPattern(
            tokens = emptyList(),
            exactIds = setOf("big-pickle"),
            capabilities = setOf(AiCapability.REASONING, AiCapability.STREAMING)
        ),
        // DeepSeek Chat
        ModelPattern(
    ...
    fun inferCapabilities(modelId: String): Set<String> {
        ...
        val scored = patterns.mapNotNull { pattern ->
            if (pattern.exactIds.isNotEmpty()) {
                val identifiers = lower.split(Regex("\\s+"))
                if (identifiers.any(pattern.exactIds::contains)) return pattern.capabilities
