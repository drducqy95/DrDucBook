...
class CacheDownloadStateStore {
    ...
    fun markSuccess(bookUrl: String, chapterIndex: Int) {
        updateBook(bookUrl) { current ->
            current.copy(
                ...
                failureMessages = current.failureMessages - chapterIndex,
    ...
    fun markFailed(bookUrl: String, chapterIndex: Int, message: String? = null) {
        updateBook(bookUrl) { current ->
            current.copy(
                runningIndices = current.runningIndices - chapterIndex,
                pausedIndices = current.pausedIndices - chapterIndex,
                failedIndices = current.failedIndices + chapterIndex,
                failureMessages = if (message.isNullOrBlank()) {
                    current.failureMessages
                } else {
                    current.failureMessages + (chapterIndex to message.trim())
                },
                chapterProgress = current.chapterProgress - chapterIndex,
            )
        }
    ...
    fun removeChapter(bookUrl: String, chapterIndex: Int): Boolean {
        val current = state.books[bookUrl] ?: return false
        val containedState = chapterIndex in current.runningIndices ||
            chapterIndex in current.pausedIndices ||
            chapterIndex in current.failedIndices ||
            chapterIndex in current.failureMessages ||
            chapterIndex in current.chapterProgress
        if (!containedState) return false
        _stateFlow.update { state ->
            val bookState = state.books[bookUrl] ?: return@update state
            state.copy(
                books = state.books + (
                    bookUrl to bookState.copy(
                        runningIndices = bookState.runningIndices - chapterIndex,
                        pausedIndices = bookState.pausedIndices - chapterIndex,
                        failedIndices = bookState.failedIndices - chapterIndex,
                        failureMessages = bookState.failureMessages - chapterIndex,
                        chapterProgress = bookState.chapterProgress - chapterIndex,
                    )
                )
            ).recalculate()
        }
