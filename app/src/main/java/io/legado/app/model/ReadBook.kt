...
object ReadBook : CoroutineScope by MainScope(), KoinComponent {
    ...
    fun setDynamicChapterTitle(bookUrl: String, chapterIndex: Int, title: String) {
        if (bookUrl.isBlank() || title.isBlank()) return
        dynamicChapterTitles["$bookUrl#$chapterIndex"] = title
    }

    private fun dynamicChapterTitle(bookUrl: String, chapterIndex: Int): String? =
        dynamicChapterTitles["$bookUrl#$chapterIndex"]
    ...
    fun loadContent(
        ...
        Coroutine.async {
            ...
            if (addLoading(index)) {
                val content = resolveReaderContent(book, chapter)
                content?.let {
                    contentLoadFinish(
                        book,
                        chapter,
            ...
            removeLoading(index)
            if (index == durChapterIndex) {
                upMsg(appCtx.getString(R.string.load_book_content_error, it.localizedMessage.orEmpty()))
            }
    ...
    private fun startTranslationObserver(taskFlow: MutableStateFlow<TranslationChapterState>, book: Book, chapter: BookChapter) {
        ...
        val job = launch {
            taskFlow.takeWhile { state ->
                val displayState = isTranslationStateVisible(book, state)
                when (state.status) {
                    TranslationChapterStatus.Translating -> {
                        state.mixedContent?.takeIf { displayState }?.let { mixed ->
                    ...
                    TranslationChapterStatus.Translated -> {
                        state.translatedContent?.takeIf { displayState }?.let { translated ->
                            contentLoadFinish(
                                book,
                                chapter,
                                translated,
                    ...
                    TranslationChapterStatus.Failed -> {
                        ...
                        if (displayState && chapterIndex == durChapterIndex) upMsg(null)
                        false
                    }

                    TranslationChapterStatus.Cancelled -> {
                        if (displayState && chapterIndex == durChapterIndex) upMsg(null)
                        false
                    }
    ...
    private fun isTranslationStateVisible(
        book: Book,
        ...
        return book.getReaderContentMode(fallbackMode).displaysTranslationProvider(
            provider = state.key.provider,
            selectedTargetLanguage = state.key.targetLanguage,
        )
    }

    private suspend fun resolveReaderContent(
        book: Book,
        chapter: BookChapter,
        ...
        return when (val mode = book.getReaderContentMode(fallbackMode)) {
            ReaderContentMode.RAW -> originalContent
            ...
            ReaderContentMode.QUICK_TRANSLATOR -> resolveLocalReaderContent(
                ...
                translate = { source ->
                    ...
                    quickTranslationGateway.translate(
                        text = source,
                        projectTerms = (scopedTerms + dictionaryGateway.getBookDictionaries(book).pairs)
                            .distinctBy { it.original.trim() },
                        customPhonetics = scopedPhonetics,
                        pronounMode = book.getQuickTranslationPronounModeOverride(),
                    )
                },
            )
    ...
    private suspend fun resolveLocalReaderContent(
        book: Book,
        chapter: BookChapter,
    ...
    fun contentLoadFinish(
        ...
        val job = Coroutine.async(this, start = CoroutineStart.LAZY) {
            ...
            val displayTitle = dynamicChapterTitle(book.bookUrl, chapter.index)
