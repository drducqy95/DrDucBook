...
class EpubFile(var book: Book) {
    ...
    companion object : BaseLocalBookParse {
        private const val MAX_EPUB_CSS_CHARS = 1_048_576
    ...
    private fun getContent(chapter: BookChapter): String? {
        ...
        if (chapter.getVariable("epubTitleDerivedFromBody") == "true") {
            elements.select("h1, h2, h3, h4, h5, h6")
                .firstOrNull { it.text().trim().equals(chapter.title.trim(), ignoreCase = true) }
                ?.remove()
        }
        ...
        elements.select("img").forEach { image ->
            image.attributes().asList()
                .filter { it.key.startsWith("on", ignoreCase = true) }
                .forEach { image.removeAttr(it.key) }
        }
        val tag = Book.rubyTag
        if (book.getDelTag(tag)) {
            elements.select("rp, rt").remove()
        }
        return EpubReaderContentFormatter.format(elements)
    }
    ...
    private fun getBody(res: Resource, startFragmentId: String?, endFragmentId: String?): Element {
        ...
        val document = Jsoup.parse(String(res.data, mCharset))
        var bodyElement = document.body()
        val styleSheets = collectStyleSheets(document, res)
        ...
        styleSheets.takeIf(String::isNotBlank)?.let { css ->
            bodyElement.prependElement("style").appendText(css)
        }
    ...
    private fun collectStyleSheets(document: Document, res: Resource): String {
        val css = StringBuilder()
        document.select("style").forEach { style ->
            if (css.length < MAX_EPUB_CSS_CHARS) css.append(style.data()).append('\n')
        }
        document.select("link[rel~=(?i)stylesheet][href]").forEach { link ->
            if (css.length >= MAX_EPUB_CSS_CHARS) return@forEach
            val linkedHref = runCatching {
                val base = res.href.encodeURI()
                val href = link.attr("href").trim().encodeURI()
                URLDecoder.decode(URI(base).resolve(href).toString(), "UTF-8")
            }.getOrNull() ?: return@forEach
            val linkedResource = epubBook?.resources?.getByHref(linkedHref) ?: return@forEach
            val remaining = MAX_EPUB_CSS_CHARS - css.length
            css.append(String(linkedResource.data, mCharset).take(remaining)).append('\n')
        }
        return css.toString()
    }
        ...
        epubBook?.let { eBook ->
            ...
            if (refs == null || refs.isEmpty()) {
                ...
                while (i < size) {
                    ...
                    val html = String(resource.data, mCharset)
                    val resolvedTitle = EpubTitleResolver.resolve(
                        resourceTitle = resource.title,
                        html = html,
                        fallback = if (i == 0) "封面" else "--卷首--",
                    )
                    val chapter = BookChapter()
                    chapter.index = i
                    chapter.bookUrl = book.bookUrl
                    chapter.url = resource.href
                    chapter.title = resolvedTitle.value
                    if (resolvedTitle.derivedFromBody) {
                        chapter.putVariable("epubTitleDerivedFromBody", "true")
                    }
    ...
    private fun parseFirstPage(
        ...
        while (i < contents.size) {
            ...
            val chapter = BookChapter()
            val html = String(epubBook!!.resources.getByHref(content.href).data, mCharset)
            val resolvedTitle = EpubTitleResolver.resolve(
