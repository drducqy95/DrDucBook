...
object WebBook {
    ...
    suspend fun searchBookAwait(
        ...
        if (VbookPluginAdapter.canHandle(bookSource)) {
            return VbookPluginAdapter.search(bookSource, key, page)
        }
    ...
    suspend fun exploreBookAwait(
        ...
        if (VbookPluginAdapter.canHandle(bookSource)) {
            return VbookPluginAdapter.explore(bookSource, url, page)
        }
        val analyzeUrl = AnalyzeUrl(
            mUrl = url,
            key = key,
    ...
    suspend fun exploreBookWithResolvedUrl(
        ...
        if (VbookPluginAdapter.canHandle(bookSource)) {
            return url to VbookPluginAdapter.explore(bookSource, url, page)
        }
        val analyzeUrl = AnalyzeUrl(
            mUrl = url,
            page = page,
    ...
    suspend fun getBookInfoAwait(
        ...
        if (VbookPluginAdapter.canHandle(bookSource)) {
            return VbookPluginAdapter.enrichBookInfo(bookSource, book)
        }
    ...
    suspend fun getChapterListAwait(
        ...
        if (VbookPluginAdapter.canHandle(bookSource)) {
            return kotlin.runCatching { VbookPluginAdapter.chapters(bookSource, book) }
    ...
    suspend fun getContentAwait(
        ...
        nextChapterUrl: String? = null,
        needSave: Boolean = true,
        followNextPages: Boolean = true,
    ): String {
        if (VbookPluginAdapter.canHandle(bookSource)) {
            return VbookPluginAdapter.content(bookSource, book, bookChapter, needSave)
        }
        ...
        return if (bookChapter.url == book.bookUrl && !book.tocHtml.isNullOrEmpty()) {
            BookContent.analyzeContent(
                ...
                needSave = needSave,
                followNextPages = followNextPages,
            )
        } else {
            val analyzeUrl = AnalyzeUrl(
