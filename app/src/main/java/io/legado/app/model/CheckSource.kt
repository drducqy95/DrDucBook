...
object CheckSource {
    ...
    fun start(context: Context, sources: List<io.legado.app.data.entities.BookSourcePart>) {
        val selectedIds = sources.map { it.bookSourceUrl }.distinct()
        if (selectedIds.isEmpty()) return
        context.startForegroundServiceCompat(
            android.content.Intent(context, CheckSourceService::class.java).apply {
                action = IntentAction.start
                putStringArrayListExtra(
                    CheckSourceService.EXTRA_SELECTED_IDS,
                    ArrayList(selectedIds),
                )
                putExtra(CheckSourceService.EXTRA_PROFILE, profile().name)
                putExtra(CheckSourceService.EXTRA_TIMEOUT_MS, timeout)
            }
        )
    }
    ...
    fun profile(): SourceCheckProfile =
        when {
            checkContent -> SourceCheckProfile.FULL
            checkInfo -> SourceCheckProfile.STANDARD
            else -> SourceCheckProfile.QUICK
        }

    fun hasSession(context: Context): Boolean =
        CheckSourceService.hasSession(context)

    fun currentTimeoutMs(): Long = timeout

    fun currentProfileName(): String = profile().name

    fun currentSourceSelection(sources: List<io.legado.app.data.entities.BookSourcePart>): List<String> =
        sources.map { it.bookSourceUrl }.distinct()

    fun refreshSelection(context: Context) {
        context.startService<CheckSourceService> {
            action = IntentAction.resume
        }
    }
