...
object StringUtils {
    ...
    fun formatFriendlyDate(dateString: String, pattern: String = "yyyy-MM-dd"): String {
        return formatFriendlyDate(appCtx, dateString, pattern)
    }

    internal fun formatFriendlyDate(
        context: Context,
        dateString: String,
        ...
        return formatFriendlyDate(
            ...
            labels = FriendlyDateLabels(
                today = context.getString(R.string.relative_date_today),
                yesterday = context.getString(R.string.relative_date_yesterday),
                daysAgo = { context.getString(R.string.relative_date_days_ago, it) },
            )
        )
    }

    internal data class FriendlyDateLabels(
        val today: String,
    ...
    internal fun formatFriendlyDate(
        ...
        return kotlin.runCatching {
            ...
            when (daysBetween) {
                0L -> labels.today
                1L -> labels.yesterday
                in 2L..5L -> labels.daysAgo(daysBetween)
                else -> dateString
            }
        }.getOrElse {
            it.printStackTrace()
    ...
    fun formatWordCountForDisplay(context: Context, rawWordCount: String?): String? {
        return formatWordCountForDisplay(
            rawWordCount = rawWordCount,
            words = { context.getString(R.string.word_count_value, it) },
            tenThousandWords = {
                context.getString(R.string.word_count_ten_thousand_value, it)
            },
        )
    }

    internal fun formatWordCountForDisplay(
        rawWordCount: String?,
        words: (String) -> String,
        tenThousandWords: (String) -> String,
    ): String? {
        val raw = rawWordCount?.trim().orEmpty()
        if (raw.isEmpty()) return null

        Regex("""^(\d+)(?:字|words?|chữ)?$""", RegexOption.IGNORE_CASE)
            .matchEntire(raw)
            ?.let { match ->
                return words(match.groupValues[1])
            }

        Regex("""^([\d.,]+)万字$""")
            .matchEntire(raw)
            ?.let { match ->
                return tenThousandWords(match.groupValues[1])
            }
