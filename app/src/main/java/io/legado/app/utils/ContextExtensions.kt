...
fun Context.startActivityForBook(
    ...
    val intent = when {
        book.isVideo -> MainActivity.createMediaPlayerIntent(this, book.bookUrl)
...
fun Context.sendToClip(text: String) {
    val clippedText = if (text.length > MAX_CLIPBOARD_TEXT_CHARS) {
        text.take(MAX_CLIPBOARD_TEXT_CHARS) + "\n\n[Content truncated for Android clipboard]"
    } else {
        text
    }
    runCatching {
        clipboardManager.setPrimaryClip(ClipData.newPlainText(null, clippedText))
    }.onSuccess {
        longToastOnUi(
            if (clippedText.length == text.length) R.string.copy_complete
            else R.string.copy_truncated
        )
    }.onFailure {
        toastOnUi(it.localizedMessage ?: "Clipboard error")
    }
}

private const val MAX_CLIPBOARD_TEXT_CHARS = 100_000
...
fun Context.openUrl(url: String) {
    try {
        val assetReference = runCatching {
            AssetDeliveryClientContract.parseInternalUri(url)
        }.getOrNull()
        if (assetReference != null) {
            startActivity(
                MainActivity.createAssetDeliveryIntent(this, url)
...
fun Context.openUrl(uri: Uri) {
    try {
        ...
        val assetReference = runCatching {
            AssetDeliveryClientContract.parseInternalUri(rawUri)
        }.getOrNull()
        if (assetReference != null) {
            startActivity(
                MainActivity.createAssetDeliveryIntent(this, rawUri)
                    .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            )
            return
        }
