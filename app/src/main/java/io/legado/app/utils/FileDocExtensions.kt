...
data class FileDoc(
    ...
    override fun toString(): String {
        return if (uri.isContentScheme()) uri.toString() else uri.toLocalFile().path
    }
    ...
    companion object {
        ...
        fun fromDir(path: String): FileDoc {
            return fromUri(directoryUriFromPath(path), true)
        }

        fun fromFile(path: String): FileDoc {
            return fromUri(Uri.fromFile(File(path)), false)
        }
...
internal fun directoryUriFromPath(path: String): Uri {
...
private fun Uri.toLocalFile(): File {
    var rawPath = when {
        isFileScheme() -> schemeSpecificPart
            ?.takeIf(String::isNotBlank)
            ?: path
            ?: toString().removePrefix("file:")

        path?.isNotBlank() == true -> path!!
        else -> toString()
    }
    if (isFileScheme() && rawPath.startsWith("//")) {
