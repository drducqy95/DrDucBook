...
(function(isSupportFileUpload){
	...
	if(!isSupportFileUpload){
		$("#drag tbody tr:last-child td span").html('Trình duyệt không hỗ trợ tải lên bằng kéo thả');
		return;
	//更换样式
	}else{
		$("#drag tbody tr:last-child td span").html('Kéo sách hoặc phông chữ vào đây để tải lên');
	}
	...
	function handleDrop(evt){
		...
		if(errorMsgs.length>0){
			...
			if(len==1){
				...
				alert("Bạn đã chọn "+len+" tệp; chỉ "+(len - errorMsgs.length)+" tệp có thể tải lên.\nHãy chọn đúng định dạng được hỗ trợ và không dùng tên tệp trùng nhau.");
			}			
		}
...
//            var data = e.target.result;
            ...
            var webSession = sessionStorage.getItem('drducbookWebSession');
            if (webSession) XHR.setRequestHeader('Authorization', 'Bearer ' + webSession);
