  ...
  <div class="app-shell" :class="{ 'reader-shell': isReader }">
    <header class="app-header" :class="{ 'is-reader-header': isReader }">
      ...
      <nav class="desktop-nav" aria-label="Điều hướng chính">
        ...
        <button class="nav-button" @click="router.push({ name: 'translation' })">{{ t('translationDashboard') }}</button>
...
.mobile-menu { display: none; border: 0; color: white; background: transparent; font-size: 22px; cursor: pointer; }
.mobile-nav { position: fixed; top: 68px; left: 0; right: 0; z-index: 19; display: grid; padding: 10px 14px; background: rgba(13,53,57,.98); box-shadow: 0 10px 24px rgba(0,0,0,.2); }
.mobile-nav button { padding: 12px; border: 0; border-bottom: 1px solid rgba(255,255,255,.12); background: transparent; color: white; text-align: left; }
...
@media (max-width: 1024px) { .app-header { padding: 0 14px; gap: 12px; }.desktop-nav { display: none; }.brand { flex: 1; min-width: 0; }.header-search { width: 150px; }.mobile-menu { display: block; }.display-settings { grid-template-columns: 1fr; }.background-grid { grid-template-columns: repeat(2, 1fr); } }
</style>
