...
/** @deprecated: 使用`API.saveBookProgressWithBeacon`以确保在页面或者直接Tắt的情况下Lưu进度 */
const saveBookProgress = (bookProgress: BookProgress) =>
  ajax.post('saveBookProgress', bookProgress)

/**主要在直接Tắt浏览器情况下可靠发送书籍进度 */
const saveBookProgressWithBeacon = (bookProgress: BookProgress) => {
  if (!bookProgress) return
  // 常规请求可能会被Hủy 使用Fetch keep-alive 或者 navigator.sendBeacon
  navigator.sendBeacon(
    withWebSession(new URL('saveBookProgress', legado_http_entry_point)),
...
const search = (
  ...
  const socket = new WebSocket(
    withWebSession(new URL('searchBook', legado_webSocket_entry_point)),
  )
  socket.onerror = wsOnError

  socket.onopen = () => {
    socket.send(JSON.stringify({ key: searchKey, sourceUrls }))
  }
...
const debug = (
  ...
  const socket = new WebSocket(withWebSession(url))
  socket.onerror = wsOnError
  socket.onopen = () => {
