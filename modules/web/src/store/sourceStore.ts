...
export const useSourceStore = defineStore('source', {
  state: () => {
    return {
      ...
      savedSources: [] as Source[], // 批量Lưu到阅读app成功的源
      currentSource: JSON.parse(JSON.stringify(emptySource)) as Source, // 当前Sửa的源
  ...
  getters: {
    ...
    searchKey: (state): string =>
      isBookSource
        ? (state.currentSource as BookSoure)?.ruleSearch?.checkKeyWord || 'Của tôi'
        : '',
  },
  actions: {
    startDebug() {
