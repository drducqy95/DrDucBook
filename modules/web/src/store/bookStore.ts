      ...
      const fetchBookshellf_promise = API.getBookShelf().then(resp => {
        ...
        if (isSuccess === true) {
          if (
            ...
            ElMessage.info(`Dữ liệu kệ sách đã cập nhật`)
          }
          ...
          if (errorMsg.includes('还没有添加小说') && this.shelf.length > 0) {
            ElMessage.info('Sách trên kệ hiện tại đã bị xóa')
            return (this.shelf = [])
          }
          ElMessage.error(errorMsg ?? 'Backend trả về định dạng không hợp lệ!')
        }
        return this.shelf
      })
    ...
    async loadWebCatalog(
      ...
      const fetchChapterList_promise = API.getChapterList(
        ...
        if (
          ...
          ElMessage.info(`Sách ${name}: mục lục chương đã cập nhật`)
        }
        this.catalog = data
        return this.catalog
      })
